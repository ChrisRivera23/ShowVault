...
void main() {
  ...
  test(
    'separately authorizes path-free NewTek TriCaster identification',
    () async {
      late http.Request captured;
      final api = ShowVaultApi(
        client: MockClient((request) async {
          captured = request;
          return http.Response(
            '{"payload":{"commandId":"newtek-command"}}',
            202,
          );
        }),
      );
      ...
      final commandId = await api.identifyNewTekTriCaster(
        accessToken: 'access-token',
        history: history,
        proposalId: 'proposal-id',
      );

      expect(commandId, 'newtek-command');
      expect(captured.method, 'POST');
      expect(
        captured.url.path,
        '/api/v1/organizations/org-id/venues/venue-id/subnet-proposals/proposal-id/identify-newtek-tricaster',
      );
      expect(captured.body, '{"timeoutMilliseconds":500}');
    },
  );
  ...
  test('separately authorizes path-free Sony camera identification', () async {
    late http.Request captured;
    final api = ShowVaultApi(
      client: MockClient((request) async {
        captured = request;
        return http.Response('{"payload":{"commandId":"sony-command"}}', 202);
      }),
    );
    const history = RecoveryHistory(
      organizationId: 'org-id',
    ...
    final commandId = await api.identifySonyCamera(
      accessToken: 'access-token',
      history: history,
      proposalId: 'proposal-id',
    );

    expect(commandId, 'sony-command');
    expect(captured.method, 'POST');
    expect(
      captured.url.path,
      '/api/v1/organizations/org-id/venues/venue-id/subnet-proposals/proposal-id/identify-sony-camera',
    );
    expect(captured.body, '{"timeoutMilliseconds":500}');
  });

  test(
    'separately authorizes path-free Allen & Heath Qu identification',
    () async {
      late http.Request captured;
      final api = ShowVaultApi(
        client: MockClient((request) async {
          captured = request;
          return http.Response('{"payload":{"commandId":"qu-command"}}', 202);
        }),
      );
      const history = RecoveryHistory(
        organizationId: 'org-id',
      ...
      final commandId = await api.identifyAllenHeathQu(
        accessToken: 'access-token',
        history: history,
        proposalId: 'proposal-id',
      );

      expect(commandId, 'qu-command');
      expect(captured.method, 'POST');
      expect(
        captured.url.path,
        '/api/v1/organizations/org-id/venues/venue-id/subnet-proposals/proposal-id/identify-allen-heath-qu',
      );
      expect(captured.body, '{"timeoutMilliseconds":500}');
    },
  );
