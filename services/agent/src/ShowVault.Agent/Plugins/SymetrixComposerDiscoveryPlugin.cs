...
public sealed class SymetrixComposerDiscoveryPlugin(
    IOptions<AgentOptions> options,
    TimeProvider timeProvider) : ExactRootFileDiscoveryPluginBase(options, timeProvider)
{
    public const string PluginId = "showvault.symetrix-composer";

    public override AgentPluginManifest Manifest { get; } = new(
        PluginId,
        "ShowVault Symetrix Composer Site",
        "0.1.0",
    ...
    protected override IReadOnlyList<string> ConfiguredRoots =>
        Options.SymetrixComposerSiteRoots;

    protected override string ProductName => "Symetrix Composer";

    protected override bool HasExpectedStructure(string rootPath) =>
        ContainsExtension(rootPath, ".symx");
}
