    ...
    TimeProvider timeProvider) : ExactRootFileDiscoveryPluginBase(options, timeProvider)
{
    ...
    protected override IReadOnlyList<string> ConfiguredRoots => Options.ShureDesignerRoomRoots;

    protected override string ProductName => "Shure Designer";

    protected override bool HasExpectedStructure(string rootPath) =>
        ContainsExtension(rootPath, ".rdf");
}
