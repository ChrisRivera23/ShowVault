...
- PR #28 has not been marked ready or merged. No new Windows workflow dispatch occurred.
...
After explicit authorization for the bridge-merge gate, revalidate unchanged `main`, PR #28's draft/open/mergeable state, exact reviewed head `5a737d23e2d8a3bcec8ef034ea88eb1fd68169b9`, one-file diff, digest/source pin, and four successful checks. Mark only PR #28 ready and merge only that exact head, then fetch and independently verify the resulting `main` workflow bytes and merge parentage. Stop before dispatch.

The next slice must satisfy these boundaries:

1. Two authorized runs now exist and both failed at distinct gates. Run `31441006486` is not authorized for rerun. The vcpkg correction is published and green at `ddde50c`; bridge commit `5a737d2` is published in green draft PR #28. Marking it ready/merging and dispatching again remain separate explicit gates.
