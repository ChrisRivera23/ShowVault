...
After explicit authorization for bridge ready/merge, revalidate draft PR #29, exact reviewed head `a307e5499d3396027bcc67fc4856ff236542fb31`, source pin/digest, one-file diff, both green CI runs, and unchanged `main` `43242c03bb6b8ee76eec3fc9debdd6f2c6daf850`. Mark ready and merge only that exact reviewed head, then verify the merge parents and byte-identical workflow on `main`. Stop before dispatching or retrieving an artifact.

The next slice must satisfy these boundaries:

1. Three authorized Windows evidence runs now exist and all failed at distinct bounded gates. None is authorized for rerun. The native-package correction and its exact one-file bridge are published and green. PR #29 remains draft at exact head `a307e549`; bridge ready/merge is next, while a later new dispatch remains separately authorized.
