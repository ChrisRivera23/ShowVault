...
After explicit authorization, implement the smallest deterministic native-dependency correction so the controlled workflow installs the exact Auth0-required vcpkg packages before the package build. Add fail-closed tests for the package-install boundary, run focused and full local verification, and prepare a new immutable product source and deterministic one-file bridge through the same separate publication/merge/dispatch gates. Do not rerun `31443483665` or retrieve an artifact.

The next slice must satisfy these boundaries:

1. Three authorized runs now exist and all failed at distinct bounded gates. None is authorized for rerun. Run `31443483665` proved the vcpkg toolchain is selected but also proved the runner tree does not already contain `cpprestsdk`. The next local correction must install the required packages explicitly and deterministically; publication, bridge merge, and a later new dispatch remain separate gates.
