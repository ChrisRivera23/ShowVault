...
- Flutter tests: 144 passed; 1 Windows-only NTFS-junction test skipped on macOS
...
Publish the corrected product head and wait for green product CI, then locally prepare and independently verify a replacement one-file bridge pinned to that exact green source. Do not update or merge the remote bridge and do not dispatch another run without the later explicit gates.

The next slice must satisfy these boundaries:

1. The original source push, PR #26 merge, and exactly one dispatch are complete. The failed run authorized no rerun. Obtain explicit authorization for the corrected `codex/windows-packaging` push; wait for its exact CI result before preparing the replacement bridge. Updating/marking ready/merging that bridge, dispatching again, retrieving an artifact, or using controlled Windows equipment remain separate gates.
