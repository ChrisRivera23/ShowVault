...
The harness-flush bridge is merged and green on exact `main` `a900fd4a57392612e791acfaebdd3d09f31cd5ea`. The next bounded objective, after explicit authorization, is exactly one new manual `Controlled Windows evidence` run: revalidate current `main`, the exact reviewed workflow pin/digest, and the absence of a newer dispatch; dispatch once; record run/attempt/job identities; wait for completion; never rerun a failure; and stop after the bounded result without artifact retrieval or equipment access.

The next slice must satisfy these boundaries:

1. Six authorized Windows evidence runs now exist and all failed at bounded gates. None is authorized for rerun. Run `31462319694` proves the pinned dependency, full Windows verification, normal package, and proof-package rebuild paths, then categorizes installed before-app preparation as `command-exit` with no recognized harness token and zero artifacts. The buffered-output correction and its bridge are merged; exactly one new dispatch is next, while artifact and equipment gates remain separate.
...
3. Before dispatch, revalidate live `main` `a900fd4a57392612e791acfaebdd3d09f31cd5ea`, exact source pin `5707a3c7bd05eca7e2d6344ee90817261578b715`, workflow digest `fa3a8e0f57b2b2e8b72f91587f45fc1d1ededff3a167a47e505e03b0576b8f06`, manual/read-only policy, and that no newer run already exists. Dispatch exactly once, wait, and record the bounded result. Never rerun a failure; artifact retrieval remains separate and requires a successful approved run.
