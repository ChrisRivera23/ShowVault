...
After explicit authorization for exactly one manual Windows workflow dispatch, revalidate current `main` `c039d5055799efe2995ab15463db19db8fa0079e`, its exact parents, green main CI run `31452046878`, byte-identical workflow digest `d2dc4893d11baba531e4e9aada3dbe295db47801495f7c7d177959ebfb383bc9`, immutable source pin `cf76d62a867c3c3918cfb0200d429d84e9c9503a`, and the unchanged four-run Windows history. Dispatch `Controlled Windows evidence` exactly once from `main`, record the new run/job identity, and monitor it to completion. Do not rerun on failure. Stop before artifact retrieval/verification or equipment access; artifact retrieval is allowed only after a successful run and separate authorization.

The next slice must satisfy these boundaries:

1. Four authorized Windows evidence runs now exist and all failed at distinct bounded gates. None is authorized for rerun. The immutable-vcpkg bridge is merged and green on `main` `c039d505`; exactly one new manual dispatch is the next separate gate, while artifact retrieval remains separately authorized after success only.
