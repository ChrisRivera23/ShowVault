...
The harness-flush bridge is locally prepared and independently verified at exact commit `daeff1f3af1ef990c678579c4005d4bf9919ab4e`. The next bounded objective, after explicit authorization, is guarded publication of that exact bridge head to `codex/windows-evidence-bridge`, creation of a new draft PR targeting exact `main`, and waiting for its push/PR CI. Stop before marking ready, merge, workflow dispatch, artifact retrieval, or equipment access.

The next slice must satisfy these boundaries:

1. Six authorized Windows evidence runs now exist and all failed at bounded gates. None is authorized for rerun. Run `31462319694` proves the pinned dependency, full Windows verification, normal package, and proof-package rebuild paths, then categorizes installed before-app preparation as `command-exit` with no recognized harness token and zero artifacts. The buffered-output correction is published and its bridge is locally verified; bridge publication, merge, dispatch, artifact, and equipment gates remain separate.
...
3. Before publication, revalidate exact local bridge commit/parent, one-file diff, source pin, digest, live `main`, and remote bridge head. Push only exact `daeff1f3af1ef990c678579c4005d4bf9919ab4e` with the established old-head lease, create a new draft PR targeting `main`, and wait for all push/PR checks. Marking ready, merge, every later manual run, and artifact retrieval require separate authorization.
