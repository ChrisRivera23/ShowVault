...
The single authorized X5 run is complete and must not be rerun. Its corrected diagnostic narrows the failure to an installed before-app prepare process that exited nonzero without a recognized harness status token. The next bounded objective, after explicit authorization, is local-only diagnosis of that `command-exit` boundary and the smallest justified code/test correction. Stop before product publication, bridge mutation, workflow dispatch, artifact retrieval, or equipment access.

The next slice must satisfy these boundaries:

1. Six authorized Windows evidence runs now exist and all failed at bounded gates. None is authorized for rerun. Run `31462319694` proves the pinned dependency, full Windows verification, normal package, and proof-package rebuild paths, then categorizes installed before-app preparation as `command-exit` with no recognized harness token and zero artifacts. Local diagnosis/correction is next; product publication, bridge, merge, dispatch, artifact, and equipment gates remain separate.
...
3. Diagnose only the installed prepare launch/status/exit boundary represented by `command-exit`, preserve fixed path-free diagnostics, and add focused regression coverage for any justified correction. A refreshed one-file bridge and any later manual run require their own authorization. Do not invoke `verify_windows_run.dart` for failed run `31462319694`; it has no artifact and cannot satisfy the verifier's successful-run precondition.
