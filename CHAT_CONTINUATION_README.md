...
The `command-exit` diagnostic correction is published and green at exact implementation commit `5707a3c7bd05eca7e2d6344ee90817261578b715`. The next bounded objective, after explicit authorization, is local-only deterministic preparation and independent verification of a fresh one-file evidence bridge from current exact `main`, pinning that source. Stop before bridge push, PR mutation, merge, workflow dispatch, artifact retrieval, or equipment access.

The next slice must satisfy these boundaries:

1. Six authorized Windows evidence runs now exist and all failed at bounded gates. None is authorized for rerun. Run `31462319694` proves the pinned dependency, full Windows verification, normal package, and proof-package rebuild paths, then categorizes installed before-app preparation as `command-exit` with no recognized harness token and zero artifacts. The buffered-output correction is published and green; bridge, merge, dispatch, artifact, and equipment gates remain separate.
...
3. Generate the refreshed one-file bridge with `dart run tool/prepare_windows_evidence_bridge.dart 5707a3c7bd05eca7e2d6344ee90817261578b715 ../../.github/workflows/windows-evidence.yml <fresh-bridge-worktree>/.github/workflows/windows-evidence.yml`, then independently run `dart run tool/verify_windows_evidence_bridge.dart` with the same arguments, confirm the digest and exact one-file diff, commit locally, and stop. Bridge publication and every later manual run require their own authorization. Do not invoke `verify_windows_run.dart` for failed run `31462319694`.
