...
After explicit authorization for local bridge preparation, create a fresh local `codex/` evidence-bridge worktree from unchanged `origin/main`, deterministically prepare and independently verify exactly one workflow file pinning published green source `a85c7e2f4be5fef263039d8da33c30719ba5c672`, inspect the one-file diff, and commit it locally. Stop before pushing or updating the remote bridge branch/PR, merging, dispatching, or retrieving an artifact.

The next slice must satisfy these boundaries:

1. Three authorized Windows evidence runs now exist and all failed at distinct bounded gates. None is authorized for rerun. The native-package correction is published and green at exact source `a85c7e2f4be5fef263039d8da33c30719ba5c672`, but has not executed on Windows. Local bridge preparation is next; bridge publication, bridge merge, and a later new dispatch remain separately authorized.
