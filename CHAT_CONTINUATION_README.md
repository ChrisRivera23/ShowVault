...
- PR #25 remains open, draft, mergeable, and based on `codex/yamaha-dme5-dme3`; its exact head is now `a85c7e2f4be5fef263039d8da33c30719ba5c672`, with all four current API/Flutter checks green. No bridge branch, PR, workflow run, or artifact was changed.
...
- The local bridge preserves `workflow_dispatch` only, `contents: read`, the exact three action SHAs, immutable checkout with credentials disabled, Windows 2025/90-minute bounds, the five-package native install, full proof/checksum/cleanup stages, and 14-day retention. Remote `codex/windows-evidence-bridge` remains at `5a737d23e2d8a3bcec8ef034ea88eb1fd68169b9`; no bridge push, PR mutation, merge, dispatch, or artifact retrieval occurred.
...
After explicit authorization for bridge publication, revalidate exact local bridge commit `a307e5499d3396027bcc67fc4856ff236542fb31`, source pin/digest, current `main`, and remote bridge old head `5a737d23e2d8a3bcec8ef034ea88eb1fd68169b9`. Push only the exact candidate with a verified lease, open a new draft one-file bridge PR to `main`, and wait for its push/pull-request CI. Stop before marking ready, merging, dispatching, or retrieving an artifact.

The next slice must satisfy these boundaries:

1. Three authorized Windows evidence runs now exist and all failed at distinct bounded gates. None is authorized for rerun. The native-package correction is published and green, and its exact bridge is locally prepared/verified at `a307e549`, but has not executed on Windows. Bridge publication is next; bridge merge and a later new dispatch remain separately authorized.
