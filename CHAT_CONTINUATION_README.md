...
3. Publish only exact implementation commit `5707a3c` after live remote-head and diff revalidation, using the established guarded fast-forward boundary, and wait for both product CI surfaces. A refreshed one-file bridge and any later manual run require their own authorization. Do not invoke `verify_windows_run.dart` for failed run `31462319694`; it has no artifact and cannot satisfy the verifier's successful-run precondition.
