...
Implement the smallest local Windows build-boundary correction for `auth0_flutter 2.6.0`: enable a validated vcpkg toolchain before the top-level CMake `project()` call, fail closed when its required native dependency setup is absent, extend the workflow/tooling tests and operator documentation, and run all host-independent verification available on macOS. Do not push, refresh the bridge, dispatch again, or use external equipment without later explicit gates.

The next slice must satisfy these boundaries:

1. Two authorized runs now exist and both failed at distinct gates. Run `31441006486` is not authorized for rerun. Patch only the evidence-backed vcpkg/CMake/toolchain-preflight cause locally, preserve the manual/read-only workflow and immutable-source bridge boundary, and keep normal customer builds authenticated.
