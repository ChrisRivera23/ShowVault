...
After explicit authorization for exact product publication, revalidate implementation commit `cf76d62a867c3c3918cfb0200d429d84e9c9503a`, its pinned vcpkg commit/closed five-port graph, focused/full/analysis results, deterministic bridge digest, remote product old head `a85c7e2f4be5fef263039d8da33c30719ba5c672`, and unchanged `main`. Push only exact `cf76d62` to `codex/windows-packaging` with the verified old-head lease and wait for push/pull-request CI at that exact SHA. Stop before preparing or publishing a bridge, merging, dispatching, or retrieving an artifact.

The next slice must satisfy these boundaries:

1. Four authorized Windows evidence runs now exist and all failed at distinct bounded gates. None is authorized for rerun. The immutable vcpkg source correction is locally complete at exact commit `cf76d62`; product publication and CI are next, while bridge, merge, dispatch, and artifact gates remain separate.
