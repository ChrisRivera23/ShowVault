...
3. Before merge, revalidate PR #32 exact head/base, one-file diff, source pin, workflow digest, all four checks, review state, and mergeability. Mark ready and merge only exact `daeff1f3af1ef990c678579c4005d4bf9919ab4e` with the expected-head guard; verify merge parents, merged workflow identity, and automatic `main` CI. Every later manual run and artifact retrieval require separate authorization.
