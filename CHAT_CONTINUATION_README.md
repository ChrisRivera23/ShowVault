...
The single authorized post-result-channel Windows run is complete and must not be rerun. It reproduced `command-exit` without a result-file token after all pre-proof gates passed. After explicit authorization, the next bounded objective is local-only diagnosis of the installed executable boundary: inspect deployed runtime dependencies, Windows loader/native startup, argument/result-channel validation, and numeric process-exit reporting; then implement only the smallest evidence-backed correction with focused and proportional verification. Stop before product publication, bridge mutation, workflow dispatch/rerun, artifact retrieval, or equipment access.
...
3. Diagnose only the installed executable boundary before the owned result file is written. Inspect the packaged DLL/runtime closure, loader/native entry, Dart argument parsing and result-file ownership validation, and the exact numeric exit code without inferring an unobserved cause. Add focused regression coverage for any justified correction and do not invoke `verify_windows_run.dart` for failed run `31470709633`.
