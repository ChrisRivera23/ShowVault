...
After explicit authorization for the exact provenance-protected product push, revalidate the candidate branch/head and push it with a verified fast-forward lease. Wait for both push and pull-request CI for that exact SHA, record every check, and stop before creating or publishing a refreshed evidence bridge. Do not rerun `31443483665` or retrieve an artifact.

The next slice must satisfy these boundaries:

1. Three authorized runs now exist and all failed at distinct bounded gates. None is authorized for rerun. The exact native-package correction is locally committed and verified, but not published or executed on Windows. Product push is the next separate external gate; bridge publication, bridge merge, and a later new dispatch remain separately authorized.
