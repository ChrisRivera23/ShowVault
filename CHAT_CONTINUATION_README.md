...
After explicit authorization for a local native-dependency correction, determine an immutable vcpkg dependency source/baseline that still contains the pinned Auth0 2.6.0 Windows package graph, implement the smallest fail-closed correction, and add static/adversarial coverage for missing or drifted ports. Run focused tests, full Flutter regression, analysis, and deterministic bridge preparation/verification locally. Stop before pushing a product head, mutating a bridge/PR, merging, dispatching, or retrieving an artifact.

The next slice must satisfy these boundaries:

1. Four authorized Windows evidence runs now exist and all failed at distinct bounded gates. None is authorized for rerun. Run `31446842882` proves the exact five-package install command cannot use the runner's moving vcpkg snapshot because `cpprestsdk` is absent. A local immutable-dependency correction is next; every later publication, bridge, merge, dispatch, and artifact gate remains separate.
