...
- The bridge diff contains one file with 19 insertions and one deletion: it replaces the immutable source pin and carries the reviewed vcpkg preflight. The bridge branch/commit has not been pushed, no PR was created or updated, and no workflow was dispatched.
...
After explicit authorization for the bridge-publication gate, revalidate exact local bridge commit `5a737d23e2d8a3bcec8ef034ea88eb1fd68169b9`, unchanged `main`, green source `ddde50cf96bf5cd86bc3aa578b893a720b4f2840`, deterministic digest, one-file diff, and current remote bridge head. Push only the reviewed bridge head and open a new draft PR to `main`; wait for all four checks and stop before marking ready or merging.

The next slice must satisfy these boundaries:

1. Two authorized runs now exist and both failed at distinct gates. Run `31441006486` is not authorized for rerun. The vcpkg correction is published and green at `ddde50c`; local bridge commit `5a737d2` is verified but unpublished. Publishing the bridge/opening a draft PR, marking it ready/merging, and dispatching again remain separate explicit gates.
