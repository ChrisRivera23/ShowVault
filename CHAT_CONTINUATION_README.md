...
After explicit authorization for the exact product publication gate, revalidate and push the current `codex/windows-packaging` head, wait for all push and PR API/Flutter checks on that exact SHA, then locally prepare and independently verify a new one-file bridge from current `origin/main` pinned to the green source. Stop before pushing or updating the bridge.

The next slice must satisfy these boundaries:

1. Two authorized runs now exist and both failed at distinct gates. Run `31441006486` is not authorized for rerun. The evidence-backed vcpkg/CMake/toolchain-preflight cause is corrected locally in `5046ad1`; publishing the final handoff head requires explicit authorization. A bridge update, merge, or new dispatch remains a later separate gate.
