...
1. Five authorized Windows evidence runs now exist and all failed at distinct bounded gates. None is authorized for rerun. Run `31452427593` proves the pinned dependency and normal package paths but fails at installed before-app vault preparation with no deeper diagnostic and zero artifacts; local diagnosis/correction is next, while product publication, bridge, merge, dispatch, artifact, and equipment gates remain separate.
