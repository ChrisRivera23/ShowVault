...
The single authorized post-flush Windows run is complete and must not be rerun. It reproduced `command-exit` without a recognized harness token even though handled Dart harness exits now flush both streams. The next bounded objective, after explicit authorization, is local-only diagnosis of the process boundary before token capture and the smallest justified code/test correction. Stop before product publication, bridge mutation, workflow dispatch, artifact retrieval, or equipment access.
...
3. Diagnose only the installed process boundary before a fixed token reaches the PowerShell capture. Inspect native runner startup/argument propagation, plugin registration, Dart entry sequencing, Windows standard handles, and exit reporting without guessing which occurred. Preserve path-free bounded evidence, add focused regression coverage for any justified correction, and do not invoke `verify_windows_run.dart` for failed run `31467029026`.
