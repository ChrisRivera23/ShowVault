...
- Ninth Windows evidence run: `31473326332`, attempt 1, job `93721325961`, failed during installed before-app preparation with bounded category `command-exit-before-harness-entry-0xc0000135`; exact dependencies, Windows verification, normal packaging, and the proof's before-package rebuild passed; zero artifacts and no rerun
...
- After exact live main/workflow/run-list revalidation, exactly one authorized manual run was dispatched: run `31473326332`, attempt 1, job `93721325961`, on exact `main` `1a71231b02a3bac2136d041ef82f36f2e144c329` and immutable product source `4c2f58b638cd2c3fd6dd10dfa047b0e8c4538e02`.
- Exact checkout, pinned Flutter/toolchain setup, pinned-vcpkg five-package installation, full Windows Flutter verification, the normal package build, and the proof's synthetic before-package rebuild and installer compilation all passed.
- The installed before-app prepare process exited before the owned Dart harness-entry marker with unsigned status `0xc0000135`. The runner failed at `tool/run-windows-installed-proof.ps1` line 113 with exact bounded category `command-exit-before-harness-entry-0xc0000135`. This establishes a Windows loader/startup boundary before Dart harness entry; it does not identify the missing runtime component by itself.
- Provenance, checksum/cleanup verification, and upload were skipped. GitHub reports zero artifacts. Run `31473326332` was not rerun, no artifact was downloaded, and no equipment was accessed. Windows readiness remains unclaimed.
...
- Windows-native run `31473326332`: exact source/toolchain/dependencies, full Windows verification, normal packaging, and proof before-package rebuild/installer compilation passed; installed before-app preparation exited before Dart harness entry with bounded status `0xc0000135`; provenance/checksum/upload were skipped, GitHub reports zero artifacts, and no rerun occurred
...
Controlled run `31473326332` is complete with exact pre-harness Windows exit `0xc0000135` and zero artifacts. After explicit authorization, the next bounded objective is to diagnose the packaged runtime closure locally, identify the evidence-backed missing Windows runtime dependency or deployment defect, implement the narrow correction, and run focused plus full local verification. Stop before product publication, bridge mutation, workflow dispatch/rerun, artifact retrieval, or equipment access.

The next slice must satisfy these boundaries:

1. Nine authorized Windows evidence runs now exist and all failed at bounded gates. None is authorized for rerun. Run `31473326332` confirms the pinned dependency, full Windows verification, normal package, and proof-package rebuild paths, then exits before Dart harness entry with `0xc0000135` and zero artifacts. Diagnose the runtime closure from repository/package evidence; do not guess which DLL or component is absent.
...
3. Keep diagnosis and correction local until separately authorized for product publication. Do not dispatch or rerun a workflow, and do not invoke `verify_windows_run.dart` for failed run `31473326332`.
