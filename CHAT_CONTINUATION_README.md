...
- No second Windows workflow dispatch has occurred. Dispatching exactly one replacement run is the next separate explicit gate; artifact retrieval remains a later gate even if that run succeeds.
...
After explicit authorization for exactly one replacement manual run, revalidate the merged bridge on current `main`, dispatch it once, wait for completion, and record the exact native result. Do not retrieve an artifact or rerun a failure without the later explicit gates.

The next slice must satisfy these boundaries:

1. The corrected source publication and PR #27 merge are complete. Exactly one failed dispatch exists. Obtain explicit authorization for one new `workflow_dispatch` run from exact current `main` `23ec1feda998fda1d1402a8f547bd7400e31f7d0`; verify the merged workflow still pins exact source `6d6f47ac3b32041e56238060b8b2cd8e13485a2d` before dispatch. Do not rerun a failure.
