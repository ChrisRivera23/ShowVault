...
After explicit authorization for the bridge ready/merge gate, revalidate unchanged remote `main` `b6d5aff28a310f3ccc3d7a6e1b38c2589170d5fa`, product source `cf76d62a867c3c3918cfb0200d429d84e9c9503a`, PR #30 open/draft/mergeable at exact head `35b5d68b6a3bb5e25a3a240186b173a6582fba13`, its one-file 78-insertion/5-deletion diff, digest `d2dc4893d11baba531e4e9aada3dbe295db47801495f7c7d177959ebfb383bc9`, and all four current green checks. Mark only that revision ready and merge it with an expected-head guard, then verify the resulting `main` parents, byte identity, source pin, and digest. Stop before any Windows workflow dispatch, artifact retrieval, or equipment access.

The next slice must satisfy these boundaries:

1. Four authorized Windows evidence runs now exist and all failed at distinct bounded gates. None is authorized for rerun. The immutable vcpkg source correction is published and green at exact commit `cf76d62`; draft PR #30 publishes its exact green one-file bridge at `35b5d68`, while ready/merge, dispatch, and artifact gates remain separate.
