...
- No third Windows workflow dispatch has occurred. Dispatching exactly one vcpkg-corrected run is the next separate explicit gate; artifact retrieval remains a later gate even if that run succeeds.
...
After explicit authorization for exactly one new manual Windows run, revalidate current `main` `43242c03bb6b8ee76eec3fc9debdd6f2c6daf850`, merge parentage, workflow digest/source pin, and the absence of a third run. Dispatch the manual workflow exactly once, wait for completion, and record exact native phase outcomes. Do not rerun a failure or retrieve an artifact without later explicit gates.

The next slice must satisfy these boundaries:

1. Two authorized runs now exist and both failed at distinct gates. Neither is authorized for rerun. The vcpkg correction is published and green at `ddde50c`, and bridge commit `5a737d2` is merged on current `main` `43242c0`. Exactly one new dispatch is the next separate explicit gate.
