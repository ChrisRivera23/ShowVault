...
- `codex/meyer-sound-projects` — Meyer Sound MAPP 3D project discovery and complete exact-root preservation.
...
- Current branch: `codex/meyer-sound-projects`, based on the L-Acoustics branch. It requires a non-empty top-level MAPP 3D `.mapp` project and preserves the exact operator-approved root, including the documented `MAPP Backup` autosave folder, versioned projects, imported DXF/SKP venue drawings, exports, and restore notes. Empty projects, backup-only or drawing-only lookalikes, and unapproved child roots do not qualify. Exact MAPP 3D and loudspeaker-data versions, venue geometry, Galileo GALAXY processor state, and deliberate live synchronization remain supervised restore boundaries.
...
- Next recommended implementation step: continue the Version 1 integration catalog with NEXO while preparing a deployed-environment and real-venue pilot.
...
- Branch: `codex/meyer-sound-projects`
- Clean worktree after feature commit `0886cad` and the README handoff commit
- Verified baseline: 230 Agent tests, 7 API tests, Flutter analysis, and 6 Flutter tests passing
- Next catalog target: NEXO
