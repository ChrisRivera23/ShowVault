...
- `codex/soundcraft-vi-showfolders` — complete Soundcraft Vi showfolders and recovery state.
...
13. Implement and pilot the approved Version 1 vendor integrations. — Priority and representative foundations implemented; remaining professional-audio manufacturer wave now covers DiGiCo SD/Quantum, SSL Live, Lawo mc², Calrec Apollo/Artemis, Studer Vista, Midas PRO Series/XL8, Behringer WING, and Soundcraft Vi
...
- Current branch: `codex/soundcraft-vi-showfolders`, based on the Behringer branch. It adds exact-root recovery for Vi showfolders with the required `Snapshots` directory and native `.snp` data, preserving ancillary files and `.bk1`/`.bk2` restart backups. Vi console/software version, surface bay count and GEQ display mode, installed hardware, and newer-to-older software compatibility remain supervised restore boundaries. Si and Ui show exports remain separate targets.
...
- Next recommended implementation step: continue the professional-audio manufacturer wave with Tascam after verifying its current recorder/mixer project formats, recovery unit boundaries, and model/firmware compatibility from official documentation.
