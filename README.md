...
- LIV nightclub is approved as the first real-venue pilot with grandMA2, Yamaha DM7 Compact, Resolume on macOS, and an L-Acoustics sound system.
...
- `codex/macos-launchdaemon-packaging` — production-style macOS LaunchDaemon packaging and dedicated service-Keychain validation for the LIV pilot.

The Auth0 Native application and live local macOS login/API proof are complete. A reproducible local prototype runbook is available in `docs/PROTOTYPE_RUNBOOK.md`. LIV nightclub is the approved first pilot, and the initial macOS LaunchDaemon package is ready for privileged installation validation on the selected pilot Mac. Membership administration, user-requested command cancellation, digital signatures, NAS/cloud storage, and persistent control-plane package records have not been implemented yet.
...
- Current branch: `codex/macos-launchdaemon-packaging`, based on the DAS Audio branch. It adds self-contained Apple-silicon/Intel package staging, a hidden `_showvault` account, exact filesystem ownership, a dedicated Keychain that is reopened after locking, one-shot enrollment without persisting the code, a root-owned LaunchDaemon, and a restart/Keychain validator. LIV nightclub is the approved pilot with grandMA2, Yamaha DM7 Compact, Resolume on macOS, and L-Acoustics. Privileged installation, reboot-without-login validation, and controlled real-artifact recovery remain to be performed on the selected pilot Mac.
...
- Next recommended implementation step: collect the LIV pilot Mac/version/storage details, perform the attended privileged LaunchDaemon installation, validate restart and logged-out reboot behavior, then execute controlled recovery proofs for Resolume, grandMA2, Yamaha DM7 Compact, and L-Acoustics. Resume the catalog with KV2 Audio after pilot blockers are addressed.
...
```text
Open and read /Users/infamous/Documents/ChatGPT/showvault/README.md in full, then inspect the repository status and current branch. Continue from the documented LIV pilot step. Review docs/LIV_PILOT.md and docs/MACOS_AGENT_INSTALL.md, collect or confirm the pilot Mac architecture, macOS version, storage and exact approved recovery roots, then perform the attended privileged LaunchDaemon installation and validation only with explicit authorization for that host. Preserve all existing user changes, keep live-device synchronization outside the first controlled restore proof, run proportional tests, and update the README handoff at significant milestones.
```

Expected starting state:

- Repository: `/Users/infamous/Documents/ChatGPT/showvault`
- Branch: `codex/macos-launchdaemon-packaging`
- Clean worktree after the macOS LaunchDaemon package and README handoff commit
- Verified baseline: 253 Agent tests, macOS plist/shell validation, successful self-contained `osx-arm64` package staging, 7 API tests, Flutter analysis, and 6 Flutter tests passing
- Next operational target: attended installation and logged-out reboot validation on the selected LIV pilot Mac
- Next catalog target after pilot blockers: KV2 Audio
...
Draft PRs #3 through #11 establish Auth0 tenancy, the Agent-side Scan → Backup → Verify → Restore loop, and tenant recovery history. Later stacked branches add approved vendor recovery integrations through DAS Audio ALMA; RCF, Funktion-One, Adamson, and Outline are explicitly deferred pending documented or fixture-verified complete recovery formats. LIV nightclub is the approved first pilot for grandMA2, Yamaha DM7 Compact, Resolume on macOS, and L-Acoustics. The Agent now has production-style macOS LaunchDaemon packaging with a hidden service identity, dedicated restart-safe Keychain, non-persistent one-shot enrollment, and validation tooling. The self-contained `osx-arm64` package stages successfully, the dedicated-Keychain lock/reopen test passes, and all 253 Agent tests pass. The next milestone is attended privileged installation and logged-out reboot validation on the selected pilot Mac, followed by controlled real-artifact recovery proofs. The catalog remains committed and resumes with KV2 Audio after pilot blockers are addressed. All 7 API tests, Flutter analysis, and all 6 Flutter tests remain at their previously verified baseline.
