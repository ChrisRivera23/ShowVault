...
- Local application detection is driven by a reusable catalog registry rather than product-specific host checks. It now covers Resolume Arena/Avenue, Serato DJ Pro, rekordbox, and Traktor Pro. Traktor adds documented Pro 2/3 application folders, bounded versioned root databases under `Documents/Native Instruments`, and generated content under `Music/Traktor` on macOS and Windows. Cross-platform fixtures keep installed, recoverable-data, approved, validated, and protected states distinct; catalog detection alone does not claim protection.
...
- `codex/traktor-catalog-detection` — catalog-driven Traktor Pro detection with cross-platform fixtures.
...
- Next recommended implementation step: add catalog-driven VirtualDJ detection for official documented macOS and Windows application and per-user recovery-data locations with cross-platform automated fixtures; do not add a product-specific scanner or claim validation/protection.
...
- Latest feature commit: `476a43d feat: detect catalog Traktor Pro locations`
...
- Next operational target: catalog-driven VirtualDJ detection with documented macOS/Windows locations and cross-platform fixtures
...
The repository is authoritative. The standing goal is zero-entry discovery of supported venue technology on older macOS/Windows systems and direct Ethernet while preserving Agent-local evidence and explicit authorization. The catalog registry now covers Resolume Arena/Avenue, Serato DJ Pro, rekordbox, and Traktor Pro. Traktor uses documented Pro 2/3 application folders, versioned root databases, and generated-content locations on both platforms; it remains detection-only. Real hardware and installed-application testing is deferred. The verified baseline is 2 contract tests, 21 platform tests, 296 Agent tests, 7 API tests, Flutter analysis, and 15 Flutter tests passing, with no pending EF Core model changes. Next is catalog-driven VirtualDJ detection using official documented standard locations and cross-platform automated fixtures.
