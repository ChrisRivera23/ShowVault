...
- `codex/catalog-local-app-detection` — reusable catalog-driven Resolume and Serato local application/data detection.
...
- Next recommended implementation step: research official KV2 Audio Control & Diagnostics documentation for an exact portable configuration or preset recovery anchor and storage boundary. Implement a fixture-backed exact-root plugin only if primary documentation establishes a dependable format; otherwise record a deliberate deferral without inferring support from generic files or network reachability.
...
- Latest feature commit: `9699fa0 feat: add catalog-driven local app detection`
...
- Next operational target: official-source KV2 Audio recovery-format research and a fixture-backed exact-root implementation or deliberate deferral
...
Draft PRs #3 through #11 establish Auth0 tenancy, the Agent-side Scan → Backup → Verify → Restore loop, and tenant recovery history. Later stacked branches add approved vendor recovery integrations through DAS Audio ALMA; RCF, Funktion-One, Adamson, and Outline are explicitly deferred pending documented or fixture-verified complete recovery formats. No pilot venue is selected. The repository is authoritative. The standing goal is to start ShowVault at an unknown venue and discover supported catalog equipment/software without pre-entered paths, addresses, models, or vendor inventory, including on documented older Macs/Windows PCs and direct laptop-to-device Ethernet. Protocols 1.3-1.12 provide exact approval, bounded reachability, documented grandMA3/Yamaha/grandMA2 identification, one unambiguous link-local Ethernet scope, passive ARP prioritization, and path-free diagnostics. Cross-platform direct-link fixtures pass through the durable queue and product identification without publishing local evidence. Local application detection now uses a catalog registry for Resolume Arena/Avenue and Serato DJ Pro standard macOS/Windows locations; Serato detection does not yet imply validation or protection. Real hardware and installed-application testing is deferred until explicit Product Owner authorization. The verified baseline is 2 contract tests, 21 platform tests, 285 Agent tests, 7 API tests, Flutter analysis, and 14 Flutter tests passing, with no pending EF Core model changes. Next is official-source KV2 Audio recovery-format research followed by an exact-root fixture-backed implementation or deliberate deferral.
