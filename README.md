...
- `codex/subnet-proposal-approval` — tenant-scoped subnet proposal review and Agent-local approval without discovery authority.
...
- Next recommended implementation step: add a separate explicit discovery authorization for one approved Agent-local subnet and implement a conservatively bounded, non-invasive first discovery pass with strict host/time limits and no banner collection or live synchronization. Manual endpoints remain an advanced fallback; arbitrary routed-network sweeps stay prohibited. Resume the catalog with KV2 Audio after zero-configuration onboarding is proven.
