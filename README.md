...
```text
Open and read /Users/infamous/Documents/ChatGPT/showvault/README.md and docs/AUTOMATIC_DISCOVERY.md in full, then inspect the repository status and current branch. Continue the venue-agnostic onboarding path: persist privacy-preserving recovery candidates and operator approvals, surface them in the native client, and convert approvals into narrow Agent-local scopes without requiring manual path entry. Preserve all existing user changes, keep arbitrary network sweeps and live-device synchronization out of scope, run proportional tests, and update the README handoff at significant milestones.
```
