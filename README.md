...
- `codex/l-acoustics-identification-research` — official-evidence review and explicit deferral of L-Acoustics network identification pending a terms-authorized read-only API contract and representative fixture.
...
- Next recommended implementation step: research official primary evidence for a read-only grandMA2 network signature and implement a separately authorized bounded identification slice only if defensible. Do not reuse grandMA3 assumptions or infer grandMA2 from an open port, generic HTTP, a banner, or reachability. Manual endpoints remain an advanced fallback; arbitrary routed-network sweeps stay prohibited. Resume the catalog with KV2 Audio after zero-configuration onboarding is proven.
...
```text
Open and read /Users/infamous/Documents/ChatGPT/showvault/README.md and docs/AUTOMATIC_DISCOVERY.md in full, then inspect the repository status and current branch. Use the completed-work list, branch history, expected starting state, and handoff snapshot as authoritative memory of everything completed in earlier conversations; do not repeat finished work. Continue the venue-agnostic onboarding path by researching official grandMA2 protocol evidence and implementing a separately authorized bounded identification only if a safe read-only primary signature is documented. Follow the documented workflow: announce the bounded outcome, inspect existing implementation and tests, use official primary sources for protocol decisions, preserve unrelated changes, implement one tested vertical slice, run proportional verification, commit the feature, and update/commit the README handoff separately when project state changes. Keep responding addresses local, require primary protocol evidence, and prohibit arbitrary routed-network sweeps, banner collection, live-device synchronization, generic HTTP, grandMA3 assumptions, and generic reachability as grandMA2 product support. In every final response, summarize what was completed, important files or commits, tests run, remaining limitations, and the next recommended task. Always finish with an explicitly labeled approximate conversation-context usage percentage and a recommendation about whether to start a new conversation; exact context usage is unavailable, so provide an honest estimate.
```

Expected starting state:

- Repository: `/Users/infamous/Documents/ChatGPT/showvault`
- Branch: `codex/l-acoustics-identification-research`
- Clean worktree after the L-Acoustics research-decision and documentation handoff commits, except the pre-existing untracked `NEXT_CONVERSATION.md`
- Latest feature/decision commit: `af31725 docs: defer L-Acoustics network identification`
- Verified baseline: 2 contract tests, 269 Agent tests, 7 API tests, Flutter analysis, and 13 Flutter tests passing; EF Core reports no pending model changes
- Next operational target: official-evidence research and a bounded grandMA2 identification slice if defensible
...
Draft PRs #3 through #11 establish Auth0 tenancy, the Agent-side Scan → Backup → Verify → Restore loop, and tenant recovery history. Later stacked branches add approved vendor recovery integrations through DAS Audio ALMA; RCF, Funktion-One, Adamson, and Outline are explicitly deferred pending documented or fixture-verified complete recovery formats. No pilot venue is selected. ShowVault must install at an unknown venue and discover candidates without pre-entered computer details, paths, addresses, or inventory. Protocols 1.3-1.6 provide path-free recovery-candidate approval/validation/backup, tenant-scoped private subnet approval, bounded reachability authorization, and Agent-local responder retention. Protocol 1.7 separately authorizes grandMA3 identification against one exact retained responder set. Protocol 1.8 separately authorizes Yamaha DME7 identification using Yamaha's documented product/manufacturer queries on TCP 49280 and exact `DME7` plus `Yamaha Corporation` responses. Both workflows retain addresses locally and have independent tenant-scoped state, command correlation, bounded evidence, errors, timestamps, native actions, automatic polling, and review displays; neither can overwrite the other. They do not authenticate, collect arbitrary banners, synchronize state, or treat an open port as support. Official L-Acoustics research found no publicly documented safe read-only wire signature; the available Electronics HTTP API contract requires separate registration and terms acceptance, so no probe or protocol change was added. Production-style macOS LaunchDaemon packaging remains available. The last code baseline remains 2 contract tests, 269 Agent tests, 7 API tests, Flutter analysis, and 13 Flutter tests passing, with no pending EF Core model changes; this documentation-only decision did not require rerunning code suites. The next milestone is official-evidence research and a bounded grandMA2 identification slice only if defensible. The catalog remains committed and resumes with KV2 Audio after zero-configuration onboarding is proven.
