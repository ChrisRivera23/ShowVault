...
- `codex/link-local-passive-observation` — cancellable passive-neighbor observation and exact direct-link retry guidance.
...
- Next recommended implementation step: validate grandMA2 identification against representative automated Telnet Remote fixtures for enabled, disabled, partial, and generic-banner cases. Preserve zero-byte probing, the 4,096-byte response cap, 100–500 ms timeout, exact responder authorization, and the requirement for the documented guest/login-prompt combination; do not contact real consoles or onPC systems.
...
- Verified baseline: 2 contract tests, 21 platform tests, 288 Agent tests, 7 API tests, Flutter analysis, and 15 Flutter tests passing; EF Core reports no pending model changes
- Next operational target: representative automated grandMA2 Telnet Remote identification fixtures
