...
- Local application detection is driven by a reusable catalog registry rather than product-specific host checks. It now covers Resolume Arena/Avenue, Serato DJ Pro, rekordbox, Traktor Pro, and VirtualDJ. VirtualDJ adds its standard macOS/Windows application plus current `Library/Application Support/VirtualDJ` or `AppData/Local/VirtualDJ` home and legacy `Documents/VirtualDJ` home on both platforms. Cross-platform fixtures keep installed, recoverable-data, approved, validated, and protected states distinct; catalog detection alone does not claim protection.
...
- `codex/virtualdj-catalog-detection` — catalog-driven VirtualDJ detection with cross-platform fixtures.
...
- Next operational target: official-source research followed by catalog-driven Engine DJ Desktop detection with documented macOS/Windows locations and cross-platform fixtures
...
The repository is authoritative. The standing goal is zero-entry discovery of supported venue technology on older macOS/Windows systems and direct Ethernet while preserving Agent-local evidence and explicit authorization. The catalog registry now covers Resolume Arena/Avenue, Serato DJ Pro, rekordbox, Traktor Pro, and VirtualDJ. VirtualDJ uses documented macOS/Windows application locations plus current and legacy per-user home locations; it remains detection-only, and custom home folders and external-drive databases are not enumerated. Real hardware and installed-application testing is deferred. The verified baseline is 2 contract tests, 21 platform tests, 296 Agent tests, 7 API tests, Flutter analysis, and 15 Flutter tests passing, with no pending EF Core model changes. Next is official-source research followed by catalog-driven Engine DJ Desktop detection using documented standard locations and cross-platform automated fixtures.
