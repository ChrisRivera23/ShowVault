...
- Local application detection is driven by a reusable catalog registry rather than product-specific host checks. It now covers Resolume Arena/Avenue, Serato DJ Pro, rekordbox, Traktor Pro, VirtualDJ, Engine DJ Desktop, and Algoriddim djay Pro. djay Pro adds its current macOS application, Windows Store package, portable `Music/djay` app data, and documented platform-specific analysis/settings roots. Cross-platform fixtures keep installed, recoverable-data, approved, validated, and protected states distinct; catalog detection alone does not claim protection.
...
- `codex/djay-pro-catalog-detection` — catalog-driven Algoriddim djay Pro detection with cross-platform fixtures.
...
- Next recommended implementation step: research official Mixxx primary sources, then add catalog-driven Mixxx detection for documented standard macOS and Windows application and per-user recovery-data locations with cross-platform automated fixtures; do not add a product-specific scanner or claim validation/protection.
...
- Next operational target: official-source research followed by catalog-driven Mixxx detection with documented macOS/Windows locations and cross-platform fixtures
...
The repository is authoritative. The standing goal is zero-entry discovery of supported venue technology on older macOS/Windows systems and direct Ethernet while preserving Agent-local evidence and explicit authorization. The catalog registry now covers Resolume Arena/Avenue, Serato DJ Pro, rekordbox, Traktor Pro, VirtualDJ, Engine DJ Desktop, and Algoriddim djay Pro. djay Pro uses its current macOS application, Windows Store package, `Music/djay` app-data root, and documented platform-specific analysis/settings roots; it remains detection-only, and legacy djay Pro 1/2 plus removable OneLibrary locations are not enumerated. Real hardware and installed-application testing is deferred. The verified baseline is 2 contract tests, 21 platform tests, 296 Agent tests, 7 API tests, Flutter analysis, and 15 Flutter tests passing, with no pending EF Core model changes. Next is official-source research followed by catalog-driven Mixxx detection using documented standard locations and cross-platform automated fixtures.
