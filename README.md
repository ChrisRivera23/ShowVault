...
- `codex/bounded-approved-subnet-discovery` — separately authorized, count-only ICMP discovery for one approved Agent-local subnet.
...
- Bounded approved-subnet discovery branch: `codex/bounded-approved-subnet-discovery`, based on subnet proposal approval. Protocol 1.6 requires a second manager action, queues only an opaque proposal ID plus hard bounds, resolves the CIDR from Agent-local approval, probes no more than 32 usable hosts via ICMP at 100-500 ms with concurrency eight, and correlates count-only outcomes to the exact authorization. No ports, banners, responding addresses, routed networks, product claims, or synchronization are included.
...
- Next recommended implementation step: retain the bounded responding-host set only in Agent-local storage, correlated to its exact authorization, then add separately authorized protocol-aware identification beginning with MA Lighting without treating ICMP or an open port as product support. Manual endpoints remain an advanced fallback; arbitrary routed-network sweeps stay prohibited. Resume the catalog with KV2 Audio after zero-configuration onboarding is proven.
...
```text
Open and read /Users/infamous/Documents/ChatGPT/showvault/README.md and docs/AUTOMATIC_DISCOVERY.md in full, then inspect the repository status and current branch. Use the completed-work list, branch history, expected starting state, and handoff snapshot as authoritative memory of everything completed in earlier conversations; do not repeat finished work. Continue the venue-agnostic onboarding path: retain the bounded responding-host set only in Agent-local storage, correlated to its exact authorization, then add a separately authorized protocol-aware MA Lighting identification slice. Follow the documented workflow: announce the bounded outcome, inspect existing implementation and tests, preserve unrelated changes, implement one tested vertical slice, run proportional verification, commit the feature, and update/commit the README handoff separately when project state changes. Keep responding addresses local, require primary protocol evidence, and prohibit arbitrary routed-network sweeps, banner collection, live-device synchronization, and generic reachability as product support. In every final response, summarize what was completed, important files and commits, tests run, remaining limitations, and the next recommended task. Always finish with an explicitly labeled approximate conversation-context usage percentage and a recommendation about whether to start a new conversation; exact context usage is unavailable, so provide an honest estimate.
```
...
- Latest feature commit: `6c15296 feat: authorize bounded subnet discovery`
- Verified baseline: 2 contract tests, 267 Agent tests, 7 API tests, Flutter analysis, and 11 Flutter tests passing; EF Core reports no pending model changes
- Next operational target: Agent-local responding-host correlation followed by separately authorized protocol-aware MA Lighting identification
...
Draft PRs #3 through #11 establish Auth0 tenancy, the Agent-side Scan → Backup → Verify → Restore loop, and tenant recovery history. Later stacked branches add approved vendor recovery integrations through DAS Audio ALMA; RCF, Funktion-One, Adamson, and Outline are explicitly deferred pending documented or fixture-verified complete recovery formats. No pilot venue is selected. ShowVault must install at an unknown venue and discover candidates without pre-entered computer details, paths, addresses, or inventory. Protocols 1.3-1.5 provide path-free recovery-candidate approval/validation/backup and tenant-scoped private subnet proposal approval. Protocol 1.6 adds a second manager authorization for one approved Agent-local subnet, caps ICMP reachability checks at 32 usable hosts, 500 ms, and concurrency eight, and publishes only attempted/responding counts correlated to the exact command. It opens no ports, retains no responding addresses, and claims no product support. Production-style macOS LaunchDaemon packaging remains available. All 2 contract tests, 267 Agent tests, 7 API tests, Flutter analysis, and 11 Flutter tests pass; EF Core reports no pending model changes. The next milestone is Agent-local responding-host retention followed by separately authorized, primary-evidence MA Lighting identification. The catalog remains committed and resumes with KV2 Audio after zero-configuration onboarding is proven.
