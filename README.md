...
- `codex/pixera-catalog-detection` — documented versioned native Windows Program Files PIXERA installation detection.
...
- Next recommended implementation step: research official Millumin primary sources, then add catalog-driven macOS installed-application and project-root detection only for documented stable standard locations with synthetic fixtures; if primary sources do not establish dependable paths, record an evidence-backed deferral instead of guessing or inspecting a real workstation.
...
- Next operational target: official-source research followed by documented-path-only Millumin macOS application/project detection, or an evidence-backed deferral if stable locations are not published
