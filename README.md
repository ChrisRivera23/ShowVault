...
- `codex/jbl-performance-manager-venues` — JBL Venue Synthesis project discovery and complete exact-root preservation.
...
- Current branch: `codex/jbl-performance-manager-venues`, based on the RCF research branch. It requires a non-empty top-level JBL Venue Synthesis `.vysn` project and preserves the exact operator-approved root, including revisions, `.lac3` Line Array Calculator designs, `.al` ArrayLink deployment files, venue models, EASE exports, reports, and restore notes. Empty projects, LAC-only or ArrayLink-only lookalikes, and unapproved child roots do not qualify. Exact Venue Synthesis/Performance versions, supported system groups, device firmware, HControl identities, array and DSP parameters, and deliberate online synchronization remain supervised restore boundaries.
...
- Next recommended implementation step: continue the Version 1 integration catalog with Martin Audio while preparing a deployed-environment and real-venue pilot.
...
- Branch: `codex/jbl-performance-manager-venues`
- Clean worktree after feature commit `fd87af4` and the README handoff commit
- Verified baseline: 242 Agent tests, 7 API tests, Flutter analysis, and 6 Flutter tests passing
- Next catalog target: Martin Audio
