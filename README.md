...
- `codex/newtek-broadcast-identification` — bounded read-only identification for the exact official NewTek TriCaster TC1 `/version` fixture.
...
- AJA broadcast-research branch: `codex/aja-broadcast-identification`, based on the NewTek broadcast-identification branch. The research decision records that AJA's official bounded REST GET framework does not publish a model-identity parameter with literal values, while official discovery guidance relies on SSDP or mDNS and also references NMOS. No scanner, multicast listener, credentials, implementation, real-device contact, or support credit is added.
- Next recommended implementation step: research official OBS Studio primary sources for dependable automatic local application and recoverable-data roots for the streaming-and-production catalog. Preserve the existing catalog-driven, path-free control-plane boundary; do not infer support from process names, generic files, user-entered paths, or undocumented platform conventions.
