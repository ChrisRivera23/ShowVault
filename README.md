...
- `codex/blackmagic-broadcast-identification` — bounded zero-byte identification for the exact official Blackmagic Smart Videohub 16x16 fixture.
...
- Next recommended implementation step: research official Sony primary sources for an exact, read-only network identity contract for the broadcast-device catalog. Reuse an existing manager-authorized bounded responder set only if its authorization covers the documented protocol; do not treat the deferred Sony projector decision as broadcast-device evidence or infer identity from generic reachability, open ports, multicast/broadcast advertisements, or mutable control endpoints.
