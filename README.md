...
- `codex/candidate-validation-results` — persisted native validation status and path-free backup from a validated candidate.
...
- Next recommended implementation step: derive read-only local network-interface candidates, exclude loopback, link-local, tunnel, and obviously non-venue interfaces, and propose bounded directly connected venue subnets for explicit approval without scanning them. Manual endpoints remain an advanced fallback; arbitrary routed-network sweeps stay prohibited. Resume the catalog with KV2 Audio after zero-configuration onboarding is proven.
