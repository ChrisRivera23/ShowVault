...
- Next recommended implementation step: research official AJA Video Systems primary sources for an exact, read-only network identity contract for the broadcast-device catalog. Reuse an existing manager-authorized bounded responder set only if its authorization covers the documented protocol; do not infer identity from generic reachability, open ports, multicast/broadcast advertisements, web titles, or mutable control endpoints.
