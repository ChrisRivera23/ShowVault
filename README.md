...
- `codex/direct-link-fixture-harness` — macOS/Windows populated and empty direct-link fixtures through reachability and product identification.
...
- Next recommended implementation step: build a reusable catalog-driven local-application detection registry, migrate the existing Resolume standard-location checks into it, and add fixture-backed Serato DJ Pro installed-application and standard `_Serato_` data candidates as the second integration. Keep installed, recoverable-data, approved, and protected states distinct. Use automated fixtures only; do not inspect or test the Product Owner's actual installed applications yet.
...
- Latest feature commit: `ae63764 test: add cross-platform direct-link fixtures`
- Verified baseline: 2 contract tests, 21 platform tests, 282 Agent tests, 7 API tests, Flutter analysis, and 14 Flutter tests passing; EF Core reports no pending model changes
- Next operational target: reusable catalog-driven local software detection with Resolume migration and Serato fixtures
...
Draft PRs #3 through #11 establish Auth0 tenancy, the Agent-side Scan → Backup → Verify → Restore loop, and tenant recovery history. Later stacked branches add approved vendor recovery integrations through DAS Audio ALMA; RCF, Funktion-One, Adamson, and Outline are explicitly deferred pending documented or fixture-verified complete recovery formats. No pilot venue is selected. The repository is authoritative. The standing goal is to start ShowVault at an unknown venue and discover supported catalog equipment/software without pre-entered paths, addresses, models, or vendor inventory, including on documented older Macs/Windows PCs and direct laptop-to-device Ethernet. Protocols 1.3-1.12 provide exact approval, bounded reachability, documented grandMA3/Yamaha/grandMA2 identification, one unambiguous link-local Ethernet scope, passive ARP prioritization, and path-free diagnostics. Cross-platform direct-link fixtures pass through the durable queue and product identification without publishing local evidence. Real hardware and installed-application testing is deferred until explicit Product Owner authorization. The beta must broadly recognize catalog software; Resolume detection exists, while Serato remains catalog-only. The verified baseline is 2 contract tests, 21 platform tests, 282 Agent tests, 7 API tests, Flutter analysis, and 14 Flutter tests passing, with no pending EF Core model changes. Next is a reusable catalog-driven local-app registry, Resolume migration, and fixture-backed Serato detection.
