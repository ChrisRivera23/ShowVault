...
- Local recovery-candidate detection is driven by a reusable catalog registry rather than product-specific host checks. It covers Resolume Arena/Avenue, disguise Designer, Serato DJ Pro, rekordbox, Traktor Pro, VirtualDJ, Engine DJ Desktop, Algoriddim djay Pro, Mixxx, and Denon Engine OS removable media. Engine OS uses a separate identity and bounded mounted-volume locations; cross-platform fixtures keep installed, recoverable-data, approved, validated, and protected states distinct, and catalog detection alone does not claim protection.
...
- `codex/disguise-designer-catalog-detection` — documented default Windows disguise Designer project-root detection.
...
- Next recommended implementation step: research official Dataton WATCHOUT primary sources, then add catalog-driven Windows installed-application and project-root detection only for documented stable standard locations with synthetic fixtures; if primary sources do not establish dependable paths, record an evidence-backed deferral instead of guessing or inspecting a real workstation.
...
- Latest feature commit: `5a7d2dd feat: detect disguise Designer project roots`
- Verified baseline: 2 contract tests, 21 platform tests, 298 Agent tests, 7 API tests, Flutter analysis, and 15 Flutter tests passing; changed-file formatting passes and EF Core reports no pending model changes
...
- Next operational target: official-source research followed by documented-path-only Dataton WATCHOUT Windows application/project detection, or an evidence-backed deferral if stable locations are not published
