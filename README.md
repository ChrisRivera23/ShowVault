...
- `codex/allen-heath-sq-shows` — Original Allen & Heath SQ numbered show directories.
...
13. Implement and pilot the approved Version 1 vendor integrations. — Resolume, grandMA, Yamaha, Q-SYS, ETC Eos, Dante Controller, and Allen & Heath SQ foundations implemented
...
- Current branch: `codex/allen-heath-sq-shows`, based on the Dante branch. It adds exact-root recovery for original SQ show directories under `AHSQ/SHOWS`, requires the numbered `SHOW####` directory and its `SHOW.DAT` anchor, and preserves the entire show folder. SQ+, dLive, Avantis, Qu, CQ, and AHM remain separate compatibility targets; SQ+ is explicitly rejected because it uses `AHSQ/SQP-SHW`.
- Next recommended implementation step: continue the representative platform foundations with Crestron recovery after verifying current source-archive, compiled-program, configuration, and device-family boundaries from official Crestron documentation.
