...
- `codex/candidate-approval-onboarding` — privacy-preserving candidate persistence and native approve/reject onboarding, based on automatic standard-location discovery.
...
- Next recommended implementation step: deliver approved decisions back to the originating Agent and convert them into durable, exact Agent-local allowlist scopes, then let the dashboard start product validation without manual path entry. After that, derive local network interfaces and propose bounded venue subnets for one-click approval. Manual paths and endpoints remain advanced fallbacks. Resume the catalog with KV2 Audio after the zero-configuration onboarding path is proven.
