...
- `codex/ventana-catalog-research` — official-source Ventana identity research and evidence-backed ambiguity deferral.
...
- Next recommended implementation step: research official Christie projector primary sources for a read-only, model-specific network identity contract. Add bounded fixture-backed identification only if the protocol and response signature are documented; do not infer Christie identity from reachability, a generic open port, or the existing Pandoras Box application integration.
