...
- `codex/prototype-flutter-workflow` — native operator controls for the complete recovery loop.
...
- Current branch: `codex/dynacord-sonicue-projects`, based on the Lab Gruppen branch. It requires a SONICUE `.snc` project and preserves colocated speaker databases, reports, revisions, and restore documentation. Speaker databases and legacy IRIS-Net/MARC artifacts alone do not qualify. Dynacord's official SONICUE 1.6 package supplies `.snc` demo projects, while official product and firmware documentation establishes version-specific support across L, C, IX, IPX, TGX, RCM-28, and MXE devices. Exact SONICUE/device-firmware versions, hardware models, routing, loudspeaker protection, control-panel/Task Engine state, and Dante configuration remain supervised restore boundaries.
...
- Prototype completion is now the active milestone. New catalog integrations are temporarily paused—not removed or descoped—until one real Scan → Backup → Verify → Restore workflow is proven through the native client. The complete catalog remains core Version 1 launch scope and resumes immediately after prototype proof, beginning with Electro-Voice.
...
- Next recommended implementation step: create the reproducible prototype deployment/runbook, configure a deployed API and PostgreSQL database, apply migrations, connect Auth0, enroll one native Agent, and execute the complete workflow against a controlled real fixture.
- Auth0 Native application `ShowVault Flutter` is registered and its callback/logout URLs and public Client ID are configured. The local native build/API proof now reaches Auth0, but user-delegated access to the ShowVault API audience still requires resolution of the tenant authorization policy described below.
- Local prototype prerequisites are now installed: Docker Desktop 4.85.0, PostgreSQL 18 in Compose, Xcode 26.6, and CocoaPods 1.17.0. The PostgreSQL schema is migrated, the local API is healthy on loopback HTTP, and the macOS Flutter client builds and launches. Live Auth0 reaches the tenant, but the tenant currently rejects creation of the per-application user-delegated grant for `ShowVault Flutter` with “You don't have permissions to access the resource.”
...
Draft PRs #3 through #11 establish Auth0 tenancy, the Agent-side Scan → Backup → Verify → Restore loop, and tenant recovery history. Later stacked branches add approved vendor recovery integrations through Dynacord SONICUE. The prototype now has safe typed workflow endpoints and native Flutter controls for all four stages. New catalog work is paused only until the prototype is proven; the full Version 1 scope remains committed in `docs/INTEGRATION_CATALOG.md` and will resume with Electro-Voice. All 208 Agent tests, all 7 API tests, Flutter analysis, and all 6 Flutter tests pass. Reproducible deployment, live Auth0/API proof, one controlled real recovery run, and macOS LaunchDaemon keychain installer validation remain outstanding.
