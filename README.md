...
- `codex/kv2-audio-recovery-research` — official-source KV2 recovery-format research and deliberate deferral pending a verified portable artifact.
...
- Next recommended implementation step: add a short cancellable passive-neighbor observation window immediately before link-local fallback, then show bounded retry guidance when both the neighbor cache and responder set are empty. Preserve the exact approved proposal/command correlation, one-Ethernet link-local rule, 32-target cap, path-free diagnostics, and fixture-only testing.
...
```text
Open and read /Users/infamous/Documents/ChatGPT/showvault/CHAT_CONTINUATION_README.md, README.md, and docs/AUTOMATIC_DISCOVERY.md in full. Keep the standing goal authoritative: start ShowVault at an unknown nightclub, concert hall, house of worship, or similar venue and automatically recognize supported catalog equipment and software without pre-entered paths, addresses, models, or vendors, including on older Macs/Windows PCs and direct Ethernet links. Inspect Git and the committed handoff, then add a short cancellable passive-neighbor observation window immediately before link-local fallback and display bounded retry guidance when both the neighbor cache and responder set are empty. Preserve the exact approved proposal/command correlation, one-Ethernet link-local rule, 32-target cap, path-free diagnostics, and fixture-only testing. Do not contact real venue hardware or inspect the Product Owner's installed applications until explicitly authorized unless a critical blocker cannot be resolved otherwise. Follow the autonomous research, short-response, separate feature/handoff commit, and approximate context-percentage workflow in CHAT_CONTINUATION_README.md, including refreshing the continuation material at approximately 90% and again at 95%.
```

Expected starting state:

- Repository: `/Users/infamous/Documents/ChatGPT/showvault`
- Branch: `codex/kv2-audio-recovery-research`
...
- Latest research-decision commit: `39494e9 docs: defer KV2 pending verified recovery format`
- Verified baseline: 2 contract tests, 21 platform tests, 285 Agent tests, 7 API tests, Flutter analysis, and 14 Flutter tests passing; EF Core reports no pending model changes
- Next operational target: cancellable passive-neighbor observation plus empty-result retry guidance
...
Draft PRs #3 through #11 establish Auth0 tenancy, the Agent-side Scan → Backup → Verify → Restore loop, and tenant recovery history. Later stacked branches add approved vendor recovery integrations through DAS Audio ALMA; RCF, Funktion-One, Adamson, Outline, and KV2 Audio are explicitly deferred pending documented or fixture-verified complete recovery formats. No pilot venue is selected. The repository is authoritative. The standing goal is to start ShowVault at an unknown venue and discover supported catalog equipment/software without pre-entered paths, addresses, models, or vendor inventory, including on documented older Macs/Windows PCs and direct laptop-to-device Ethernet. Protocols 1.3-1.12 provide exact approval, bounded reachability, documented grandMA3/Yamaha/grandMA2 identification, one unambiguous link-local Ethernet scope, passive ARP prioritization, and path-free diagnostics. Cross-platform direct-link fixtures pass through the durable queue and product identification without publishing local evidence. Local application detection uses a catalog registry for Resolume Arena/Avenue and Serato DJ Pro standard macOS/Windows locations; Serato detection does not yet imply validation or protection. KV2 Control & Diagnostics recovery remains uncredited because official material does not expose a dependable preset/show-file contract. Real hardware and installed-application testing is deferred until explicit Product Owner authorization. The verified baseline is 2 contract tests, 21 platform tests, 285 Agent tests, 7 API tests, Flutter analysis, and 14 Flutter tests passing, with no pending EF Core model changes. Next is a cancellable passive-neighbor observation window plus empty-result retry guidance without weakening link-local authorization or privacy bounds.
