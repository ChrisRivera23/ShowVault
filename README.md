...
- The permanent product goal now explicitly requires catalog-backed discovery at unknown venues, compatibility testing for older macOS and Windows computers, and direct laptop-to-device Ethernet as a supported test topology. Current private-IPv4 direct links can use the approved bounded flow; self-assigned IPv4 link-local discovery remains a future safety-designed slice.
...
```text
Open and read /Users/infamous/Documents/ChatGPT/showvault/README.md and docs/AUTOMATIC_DISCOVERY.md in full. Keep this goal authoritative in every handoff: start ShowVault at an unknown nightclub, concert hall, house of worship, or similar venue and discover supported catalog equipment/software without pre-entered paths, addresses, models, or vendors, including on documented older Macs/Windows PCs and direct laptop-to-device Ethernet links. Inspect Git and the committed handoff, then continue by autonomously researching official grandMA2 protocol evidence and implementing separately authorized bounded identification only if a safe read-only primary signature is documented. Do not ask permission merely to search public/official sources; stop only for credentials, identity/account submission, binding terms, payment, or security-sensitive authorization. Preserve local-only addresses, exact authorization, and primary evidence; never infer grandMA2 from grandMA3 behavior, reachability, an open port, generic HTTP, or a banner. Implement one tested bounded slice, commit it, update and separately commit the handoff, and keep responses short and direct. End with completed work, verification, commits, limitations, exact next task, untouched files, estimated context usage, and a new-chat recommendation.
```
...
- Latest product-decision commit: `89c8b4f docs: add legacy and direct-link discovery goals`
...
Draft PRs #3 through #11 establish Auth0 tenancy, the Agent-side Scan → Backup → Verify → Restore loop, and tenant recovery history. Later stacked branches add approved vendor recovery integrations through DAS Audio ALMA; RCF, Funktion-One, Adamson, and Outline are explicitly deferred pending documented or fixture-verified complete recovery formats. No pilot venue is selected. The repository is authoritative. The standing goal is to start ShowVault at an unknown venue and discover supported catalog equipment/software without pre-entered paths, addresses, models, or vendor inventory, including on documented older Macs/Windows PCs and direct laptop-to-device Ethernet. Protocols 1.3-1.6 provide path-free recovery-candidate approval/validation/backup, tenant-scoped private subnet approval, bounded reachability authorization, and Agent-local responder retention. Protocol 1.7 separately authorizes grandMA3 identification, and protocol 1.8 separately authorizes Yamaha DME7 identification using exact documented evidence; addresses remain local and the workflows remain independently correlated. Official L-Acoustics research found no publicly documented safe read-only wire signature, so no probe was added. Current direct private-IPv4 links can follow the bounded flow; self-assigned IPv4 link-local discovery is not yet supported and requires a separately safety-designed slice. The last code baseline remains 2 contract tests, 269 Agent tests, 7 API tests, Flutter analysis, and 13 Flutter tests passing, with no pending EF Core model changes; these documentation-only decisions did not require rerunning code suites. The next milestone is official-evidence research and a bounded grandMA2 identification slice only if defensible. The catalog resumes with KV2 Audio after zero-configuration onboarding is proven.
