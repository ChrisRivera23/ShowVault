...
- Next recommended implementation step: persist bounded path-free grandMA3 identification status/evidence in the control plane and expose explicit operator review in native onboarding. Then extend the same separately authorized primary-evidence model to Yamaha and L-Acoustics. Manual endpoints remain an advanced fallback; arbitrary routed-network sweeps stay prohibited. Resume the catalog with KV2 Audio after zero-configuration onboarding is proven.
...
- Latest feature commit: `1b949cf feat: identify grandMA3 from authorized hosts`
...
- Next operational target: tenant-scoped, path-free grandMA3 identification results and native operator review
...
Draft PRs #3 through #11 establish Auth0 tenancy, the Agent-side Scan → Backup → Verify → Restore loop, and tenant recovery history. Later stacked branches add approved vendor recovery integrations through DAS Audio ALMA; RCF, Funktion-One, Adamson, and Outline are explicitly deferred pending documented or fixture-verified complete recovery formats. No pilot venue is selected. ShowVault must install at an unknown venue and discover candidates without pre-entered computer details, paths, addresses, or inventory. Protocols 1.3-1.6 provide path-free recovery-candidate approval/validation/backup, tenant-scoped private subnet approval, bounded reachability authorization, and Agent-local responder retention. Protocol 1.7 separately authorizes grandMA3 identification only against one exact retained responder set and requires an application signature from the officially documented Web Remote service. Addresses and matches remain Agent-local; only attempted/matched counts and the grandMA3 family are published. It performs no authentication, session enumeration, arbitrary banner collection, or synchronization, and it does not claim grandMA2 support. Production-style macOS LaunchDaemon packaging remains available. All 2 contract tests, 268 Agent tests, 7 API tests, Flutter analysis, and 11 Flutter tests pass. The next milestone is tenant-scoped path-free grandMA3 result persistence and native operator review. The catalog remains committed and resumes with KV2 Audio after zero-configuration onboarding is proven.
