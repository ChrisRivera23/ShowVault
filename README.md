...
- `codex/presonus-studiolive-series-iii-backups` — StudioLive Series III Universal Control full backups.
...
- Next recommended implementation step: continue the professional-audio manufacturer wave with Biamp after verifying current Tesira project/backup formats and compatibility. QSC is already represented by the implemented Q-SYS Designer `.qsys` recovery target.
...
- Exact Codex context-window percentages are not exposed to the assistant. The current conversation is estimated at roughly 75–83% after the PreSonus StudioLive Series III slice; this is an unmeasured estimate. This README is the durable source of truth for a new task and should be copied or referenced when starting one.
