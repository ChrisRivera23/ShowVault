...
- `codex/shure-designer-rooms` — Shure Designer 6 room designs and recovery companions.
...
13. Implement and pilot the approved Version 1 vendor integrations. — Resolume, grandMA, Yamaha, Q-SYS, ETC Eos, Dante Controller, Allen & Heath SQ, Crestron SIMPL, and Shure Designer foundations implemented
...
- Current branch: `codex/shure-designer-rooms`, based on the Crestron branch. It adds exact-root Shure Designer 6 recovery, requires a current `.rdf` room design, and preserves legacy `.dprj` projects, floor plans, reports, and non-secret deployment notes. Wireless Workbench `.shw` shows and Show Packs remain a separate live-RF recovery workflow, and device passphrases and IntelliMix license credentials must remain in an approved secret store rather than recovery packages.
- Next recommended implementation step: complete the representative platform foundations with Blackmagic Design recovery after verifying current ATEM, DaVinci Resolve, HyperDeck, and media-device project/export boundaries from official Blackmagic documentation.
