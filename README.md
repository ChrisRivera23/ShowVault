...
- Next recommended implementation step: research official DT Videolabs primary sources for dependable automatic local application and recoverable-playlist/media roots for the PlaybackPro row under Show control and playback. Preserve the existing catalog-driven, path-free control-plane boundary; do not infer support from process names, generic files, user-entered paths, or undocumented platform conventions.
