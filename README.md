...
- `codex/sony-projector-identification` — official-source decision deferring Sony model identification without an exact model response signature.
...
- Next recommended implementation step: research official Blackmagic Design primary sources for an exact, read-only network identity contract for the broadcast-device catalog. Reuse an existing manager-authorized bounded responder set if a documented signature exists; do not infer identity from generic reachability, open ports, multicast/broadcast advertisements, or mutable control endpoints.
