...
- Next recommended implementation step: research official Show Cue Systems primary sources for dependable automatic local application and recoverable-show roots for the SCS row under Show control and playback. Preserve the existing catalog-driven, path-free control-plane boundary; do not infer support from process names, generic files, user-entered paths, or undocumented platform conventions.
