...
- `codex/lab-gruppen-lake-systems` — Lab Gruppen Lake Controller `.csc` system configurations and recovery companions.
...
- Current branch: `codex/lab-gruppen-lake-systems`, based on the Crown branch. It requires the Lake Controller `.csc` whole-system configuration and preserves Contour `.csm`/`.cbm` and Mesa `.msm`/`.mbm` module/base files, revisions, reports, and restore documentation. Module files alone do not qualify. Official Lake documentation defines `.csc` as storing all module information plus frame and group assignments; exact Controller/firmware versions, compatible frame models, I/O and failover routing, groups, module assignments, loudspeaker protection, and Dante state remain supervised restore boundaries.
...
- Next recommended implementation step: start a fresh Codex task using this README, then continue the professional-audio manufacturer wave with Dynacord after verifying current SONICUE project/preset formats and amplifier/DSP compatibility from official documentation.
...
- Exact Codex context-window percentages are not exposed to the assistant. This conversation is estimated at roughly 92–95% after the Lab Gruppen slice (unmeasured); start a fresh task now and provide this README as the complete handoff.
...
Draft PRs #3 through #11 establish Auth0 tenancy, the Agent-side Scan → Backup → Verify → Restore loop, and tenant recovery history. Later stacked branches add the approved vendor recovery integrations through Ashly Protea NE, Powersoft ArmoníaPlus, Crown Audio Architect, and Lab Gruppen Lake Controller. Avid VENUE and Crest Audio remain deferred pending verified native fixtures. Dynacord is next. All 203 Agent tests pass. The full Version 1 scope remains in `docs/INTEGRATION_CATALOG.md`; live Auth0/API proof and macOS LaunchDaemon keychain installer validation remain outstanding.
