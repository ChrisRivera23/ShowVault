...
- Local application detection is now driven by a reusable catalog registry rather than Resolume-specific host checks. Resolume Arena/Avenue retain their existing application and user-data candidates, while Serato DJ Pro adds documented macOS/Windows installed-application and per-user `Music/_Serato_` recovery-data candidates. Cross-platform fixtures keep installed, recoverable-data, approved, validated, and protected states distinct; Serato validation and protection are not yet claimed.
...
- `codex/grandma2-telnet-fixtures` — automated grandMA2 console/onPC Telnet Remote fixture validation.
...
- Next recommended implementation step: add catalog-driven rekordbox detection for documented standard macOS and Windows application and per-user recovery-data locations, using official primary sources and cross-platform automated fixtures. Extend the existing registry rather than adding a product-specific scanner; keep paths Agent-local and do not claim validation, protection, or restore support.
...
```text
Open and read /Users/infamous/Documents/ChatGPT/showvault/CHAT_CONTINUATION_README.md, README.md, and docs/AUTOMATIC_DISCOVERY.md in full. Keep the standing goal authoritative: start ShowVault at an unknown nightclub, concert hall, house of worship, or similar venue and automatically recognize supported catalog equipment and software without pre-entered paths, addresses, models, or vendors, including on older Macs/Windows PCs and direct Ethernet links. Inspect Git and the committed handoff, then add catalog-driven rekordbox detection for documented standard macOS and Windows application and per-user recovery-data locations. Research official primary sources, extend the existing local-application registry and cross-platform fixtures, keep paths Agent-local, and keep installed, recoverable-data, approved, validated, and protected states distinct. Do not add a product-specific scanner, read candidate contents, inspect the Product Owner's installed applications, or claim rekordbox validation, backup, verification, or restore support. Follow the autonomous research, short-response, separate feature/handoff commit, and approximate context-percentage workflow in CHAT_CONTINUATION_README.md, including refreshing the continuation material at approximately 90% and again at 95%.
```
...
- Verified baseline: 2 contract tests, 21 platform tests, 295 Agent tests, 7 API tests, Flutter analysis, and 15 Flutter tests passing; EF Core reports no pending model changes
- Next operational target: catalog-driven rekordbox detection with documented macOS/Windows locations and cross-platform fixtures
...
Draft PRs #3 through #11 establish Auth0 tenancy, the Agent-side Scan → Backup → Verify → Restore loop, and tenant recovery history. Later stacked branches add approved vendor recovery integrations through DAS Audio ALMA; RCF, Funktion-One, Adamson, Outline, and KV2 Audio are explicitly deferred pending documented or fixture-verified complete recovery formats. No pilot venue is selected. The repository is authoritative. The standing goal is to start ShowVault at an unknown venue and discover supported catalog equipment/software without pre-entered paths, addresses, models, or vendor inventory, including on documented older Macs/Windows PCs and direct laptop-to-device Ethernet. Protocols 1.3-1.12 provide exact approval, bounded reachability, documented grandMA3/Yamaha/grandMA2 identification, one unambiguous link-local Ethernet scope, passive ARP prioritization, and path-free diagnostics. Link-local discovery observes the cache across a cancellable one-second window and revalidates the unchanged Ethernet boundary before fallback; empty exact-direct-link outcomes show retry guidance. Synthetic grandMA2 console/onPC fixtures validate the documented guest/login signature, safe disabled and silent outcomes, partial/generic/grandMA3 rejection, fragmented reads, the 4,096-byte cap, and zero-byte probing without contacting real systems. Local application detection uses a catalog registry for Resolume Arena/Avenue and Serato DJ Pro standard macOS/Windows locations; Serato detection does not yet imply validation or protection. KV2 Control & Diagnostics recovery remains uncredited because official material does not expose a dependable preset/show-file contract. Real hardware and installed-application testing is deferred until explicit Product Owner authorization. The verified baseline is 2 contract tests, 21 platform tests, 295 Agent tests, 7 API tests, Flutter analysis, and 15 Flutter tests passing, with no pending EF Core model changes. Next is catalog-driven rekordbox detection using official documented standard locations and cross-platform automated fixtures.
