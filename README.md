...
- Next recommended implementation step: research official primary sources for a bounded, read-only, exact product-identity contract for the BirdDog row under PTZ and cameras. Preserve the existing manager-authorized responder boundary and path-free control-plane contract; do not infer identity from generic NDI, VISCA, ONVIF, HTTP, mDNS, or port reachability.
