...
- `codex/bose-controlspace-projects` — Bose Professional ControlSpace Designer `.csp` projects and recovery companions.
...
- Current branch: `codex/bose-controlspace-projects`, based on the Symetrix branch. It adds exact-root recovery for native ControlSpace Designer `.csp` projects and preserves `.cpf` control panels, `.cpz` packaged remote projects, retrieved `.cab` archives, revisions, and restore documentation. Companion files alone do not qualify. Bose documentation states that `.csp` contains configuration, settings, and control functions for the system and recommends using the commissioned Designer/firmware version; hardware models and configurations, project-network/IP assignments, Dante state, and deliberate online send/save-to-flash behavior remain supervised restore boundaries.
...
- Next recommended implementation step: continue the professional-audio manufacturer wave with Peavey after verifying current NWare/NION project formats and hardware/software compatibility from official documentation.
...
- Exact Codex context-window percentages are not exposed to the assistant. Estimate conversation context conservatively and update this handoff before recommending a new task.
...
Draft PRs #3 through #11 establish Auth0 tenancy, the Agent-side Scan → Backup → Verify → Restore loop, and tenant recovery history. PR #12 adds native Flutter and Auth0; PR #13 adds system inventory; PR #14 adds protocol 1.2 allowlisted TCP discovery; PRs #15–#16 add Resolume recovery; PR #17 adds grandMA2/grandMA3; PRs #18–#20 add Yamaha DM7, RIVAGE PM, CL/QL, TF, and DM3; PR #21 adds distinct Yamaha DME7 `.pvd` and MTX/MRX `.mtx` recovery targets; PR #22 adds PC-D/DI amplifier project compatibility; PR #23 protects editable ProVisionaire Control PLUS `.pvcppj` projects and exported `.pvksk` Kiosk controllers; PR #24 adds DME5/DME3 `.pvd` compatibility and Custom Control Panel companions. Later branches add Q-SYS Designer, ETC Eos, Dante Controller, Allen & Heath SQ, Crestron SIMPL Windows, Shure Designer 6, Blackmagic ATEM, DiGiCo, SSL Live, Lawo, Calrec, Studer, Midas, Behringer WING, Soundcraft Vi, Tascam, Roland M-5000, PreSonus StudioLive Series III, Biamp Tesira, Symetrix Composer, and Bose Professional ControlSpace recovery. Avid VENUE remains deferred pending a verified export fixture. Peavey is next. All 177 Agent tests pass. The full Version 1 scope remains in `docs/INTEGRATION_CATALOG.md`; live Auth0/API proof and macOS LaunchDaemon keychain installer validation remain outstanding.
