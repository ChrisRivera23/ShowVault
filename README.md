...
- `codex/agent-local-candidate-scopes` — privacy-preserving approval delivery and durable exact Agent-local scopes, based on native candidate onboarding.
...
- Next recommended implementation step: add a typed Agent operation that starts product-specific validation from an approved local candidate ID, resolves the scope locally, and returns only path-free validation findings so onboarding no longer requires manual path entry. After that, derive local network interfaces and propose bounded venue subnets for one-click approval. Manual paths and endpoints remain advanced fallbacks. Resume the catalog with KV2 Audio after the zero-configuration onboarding path is proven.
