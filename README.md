...
- `codex/biamp-tesira-configurations` — Biamp Tesira editable configuration files and recovery companions.
...
- Next recommended implementation step: start a new Codex task using this README, then continue the professional-audio manufacturer wave with Symetrix after verifying current Composer/SymNet project formats and device/software compatibility from official documentation.
...
- Exact Codex context-window percentages are not exposed to the assistant. The current conversation is estimated at roughly 88–92% after the Biamp Tesira slice; this is an unmeasured estimate. Start a new task now and provide this README as the complete handoff.
