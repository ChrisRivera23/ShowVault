...
- Local recovery-candidate detection is driven by a reusable catalog registry rather than product-specific host checks. It covers Resolume Arena/Avenue, disguise Designer, Dataton WATCHOUT 7, Green Hippo Hippotizer V4, AV Stumpfl PIXERA, Christie Pandoras Box, Derivative TouchDesigner, Serato DJ Pro, rekordbox, Traktor Pro, VirtualDJ, Engine DJ Desktop, Algoriddim djay Pro, Mixxx, and Denon Engine OS removable media. Engine OS uses a separate identity and bounded mounted-volume locations; cross-platform fixtures keep installed, recoverable-data, approved, validated, and protected states distinct, and catalog detection alone does not claim protection.
...
- `codex/touchdesigner-catalog-detection` — official-source Derivative TouchDesigner research and documented standard macOS/Windows installation detection.
...
- Next operational target: official-source research followed by documented-path-only HeavyM application/project detection, or an evidence-backed deferral if stable locations are not published
