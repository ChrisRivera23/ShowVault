...
- Local application detection is driven by a reusable catalog registry rather than product-specific host checks. It now covers Resolume Arena/Avenue, Serato DJ Pro, rekordbox, Traktor Pro, VirtualDJ, and Engine DJ Desktop. Engine DJ adds its standard macOS/Windows application and each user's documented `Music/Engine Library` metadata database. Cross-platform fixtures keep installed, recoverable-data, approved, validated, and protected states distinct; catalog detection alone does not claim protection.
...
- `codex/engine-dj-catalog-detection` — catalog-driven Engine DJ Desktop detection with cross-platform fixtures.
...
- Next operational target: official-source research followed by catalog-driven Algoriddim djay Pro detection with documented macOS/Windows locations and cross-platform fixtures
...
The repository is authoritative. The standing goal is zero-entry discovery of supported venue technology on older macOS/Windows systems and direct Ethernet while preserving Agent-local evidence and explicit authorization. The catalog registry now covers Resolume Arena/Avenue, Serato DJ Pro, rekordbox, Traktor Pro, VirtualDJ, and Engine DJ Desktop. Engine DJ uses documented macOS/Windows application locations and the per-user `Music/Engine Library` metadata database; it remains detection-only, and removable-drive libraries and Engine OS hardware are not enumerated. Real hardware and installed-application testing is deferred. The verified baseline is 2 contract tests, 21 platform tests, 296 Agent tests, 7 API tests, Flutter analysis, and 15 Flutter tests passing, with no pending EF Core model changes. Next is official-source research followed by catalog-driven Algoriddim djay Pro detection using documented standard locations and cross-platform automated fixtures.
