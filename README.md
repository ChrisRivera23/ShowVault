...
- `codex/crestron-simpl-projects` — Crestron SIMPL Windows source projects and recovery companions.
...
13. Implement and pilot the approved Version 1 vendor integrations. — Resolume, grandMA, Yamaha, Q-SYS, ETC Eos, Dante Controller, Allen & Heath SQ, and Crestron SIMPL foundations implemented
...
- Current branch: `codex/crestron-simpl-projects`, based on the Allen & Heath branch. It adds exact-root SIMPL Windows recovery, requires editable `.smw` source, and preserves colocated `.usp` modules, user macros, IR files, VT Pro-e `.vtp` source, Smart Graphics definitions, compiled `.cpz`/`.lpz` programs, `.vtz`/`.ch5z` UI packages, drivers, and documentation. SIMPL#, Crestron Construct source, D3 Pro, Crestron Home, and `.AV Framework` remain separate compatibility targets.
- Next recommended implementation step: continue the representative platform foundations with Shure recovery after verifying current Wireless Workbench, Designer, IntelliMix, and device-preset artifacts and compatibility boundaries from official Shure documentation.
