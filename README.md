...
- The separate catalog row named Pandora's Box Manager is reconciled to the existing `showvault.christie-pandoras-box` candidate for current V8 installation. Christie states that V8's unified software license replaced the former separate Manager and other edition licenses with one combined feature suite. Candidate existence does not prove activation, enabled Manager capability, legacy standalone Manager licensing, validation, backup, verification, or restore support.
...
- PVP (ProVideoPlayer) automatic local detection is deliberately deferred. Renewed Vision documents placing the application in Applications but publishes neither an exact fixed bundle path nor a standard Show/media root; Shows exclude media, absolute media paths are operator-selected, and relative media requires an operator-created folder. ShowVault adds no bundle or Show-file guess, filesystem scan, workstation inspection, or installation/recovery support credit.
- CuePilot automatic local detection is deliberately deferred. CuePilot ApS publishes a macOS Applications workflow but no stable Windows installed path or bounded local recovery root; collaborative projects synchronize to its cloud, the local SOLO-project path is unpublished, exports are operator-located, and media is selected from arbitrary local paths or uploaded. ShowVault adds no cache/path inference, cloud access, filesystem scan, workstation inspection, or installation/recovery support credit.
- TinkerList automatic local detection is deliberately deferred. The current product is Cuez by TinkerList: its rundown and project workflow is cloud-based, official Automator guidance publishes no stable installed executable or bundle path, and downloaded Episode media is stored at an operator-defined location that may be local, remote, or temporary. ShowVault adds no executable/service/cache inference, account or cloud access, filesystem scan, workstation inspection, or installation/recovery support credit.
...
- PTZOptics automatic network identification is deliberately deferred. Official VISCA inquiry material exposes camera state but no manufacturer/model identity, while the digest-authenticated HTTP device-info endpoint covers both Move 4K and Link 4K and publishes only one example containing an editable device name, serial/firmware data, and undocumented internal codes. ShowVault adds no credentials, authentication weakening, generic-protocol inference, mDNS/Fleet Manager discovery, privacy-bearing data collection, real-camera contact, or identification/recovery support credit.
- Protocol 1.16 adds separately authorized BirdDog P200 A4/A5 identification against responders retained by one completed bounded discovery. The Agent sends only the official read-only `GET /version` request on TCP 8080, caps the body at 64 bytes, and accepts only exact `BirdDog P200A4_A5` with an optional terminal newline. Addresses and raw responses remain Agent-local; other models/revisions, altered identifiers, redirects, authentication challenges, non-200/oversized responses, generic NDI/VISCA/ONVIF/HTTP behavior, hostnames, serials, firmware, and reachability are safe false negatives. Tenant-scoped pending/completed/failed state and a native manager action are included; no real camera was contacted and no control or recovery support is credited.
- Protocol 1.17 adds separately authorized Panasonic AW-UE150A and AW-UE100 identification against responders retained by one completed bounded discovery. The Agent sends only Panasonic's official read-only AW-over-IP `QID` request on TCP 80, caps the body at 64 bytes, and accepts only exact `OID:AW-UE150A` or `OID:AW-UE100` with an optional terminal newline. Addresses and raw responses remain Agent-local; other models, projector evidence, generic AW/VISCA/ONVIF/NDI/HTTP behavior, redirects, authentication challenges, non-200/oversized responses, camera titles, versions, and reachability are safe false negatives. Tenant-scoped pending/completed/failed state and a native manager action are included; no real camera was contacted and no control or recovery support is credited.
- Protocol 1.18 adds separately authorized Sony BRC-X400/X401, SRG-X400/X402/201M2/X120/HD1M2, and SRG-A40/A12 identification against responders retained by one completed bounded discovery. The Agent sends only Sony's official read-only CGI system inquiry on TCP 80 with the documented same-host Referer, caps the privacy-bearing grouped response at 16,384 bytes, and accepts exactly one case-sensitive allowlisted `ModelName`. Addresses and raw responses remain Agent-local; credentials are never sent, 401 responses are safe false negatives, and other models, projector/broadcast evidence, generic CGI/VISCA/ONVIF/NDI/HTTP behavior, redirects, camera names, serials, versions, and reachability remain unsupported. Tenant-scoped pending/completed/failed state and a native manager action are included; no real camera was contacted and no control or recovery support is credited.
- Protocol 1.19 adds separately authorized Allen & Heath Qu-16, Qu-24, Qu-32, Qu-Pac, and Qu-SB identification against responders retained by one completed bounded discovery. The Agent sends only the official read-only MIDI-over-TCP Get System State request on TCP 51325 with the Qu-Pad flag unset, caps received data at 64 bytes, and requires the complete Allen & Heath Qu SysEx reply with an exact allowlisted `BoxID`. Addresses, firmware fields, and raw state traffic remain Agent-local; SQ/SQ+, CQ, AHM, Avantis, dLive, generic MIDI/TCP behavior, discovery broadcasts, Dante/AES67 evidence, and reachability remain unsupported. Tenant-scoped pending/completed/failed state and a native manager action are included; no real mixer was contacted and no control or recovery support is credited.
- DiGiCo automatic network identification is deliberately deferred. Official SD/Quantum and S-Series documentation requires external control to be enabled and controllers, send/receive ports, and console-specific or user-defined command sets to be configured; official 4REA4 controller discovery is part of software that can change live configuration and update firmware. No fixed, read-only request with an exact literal model response is published, so ShowVault does not probe undocumented controller protocols or infer identity from generic TCP/OSC/MIDI, Dante, Optocore, default addresses, discovery, broadcasts, or port reachability. Existing exact-root SD/Quantum `.ses` recovery remains separate.
- Avid automatic network identification is deliberately deferred. Official VENUE and EuControl software enumerate compatible systems/controllers, but expose control-capable workflows, editable device names, and UI type icons without publishing a fixed read-only wire request and exact literal model response. ECx is authenticated VNC remote control; VENUE Link and VENUE 8.2 OSC use enabled or operator-configured connections, names, ports, paths, values, and optional broadcasts. ShowVault does not probe undocumented discovery, submit credentials, connect to these control endpoints, or infer identity from generic EUCON, VENUE Link, VNC, OSC, AVB, Dante, names, icons, discovery, broadcasts, or port reachability. The existing VENUE recovery deferral remains separate.
- SSL automatic network identification is deliberately deferred. Official Live/SOLSA documentation shows discovered consoles and a manual default port, but publishes no fixed read-only discovery request or exact literal model response; connecting requires a password and console authorization, and console names are editable. SSL Network I/O discovery is tied to Dante-aware controller software that can take device/channel ownership and alter gain, phantom power, routing, settings, or firmware. ShowVault does not probe these undocumented control protocols, submit credentials, or infer identity from generic TCP/UDP, OSC, MIDI, Dante, MADI, System T/NMOS, editable names, discovery, default ports, or reachability. Existing exact-root SSL Live `.show` recovery remains separate.
- Protocol 1.20 adds separately authorized identification for the standard Behringer WING console against responders retained by one completed bounded discovery. The Agent sends only Behringer's official five-byte UDP `WING?` information request on port 2222, caps the reply at 256 bytes, and requires exactly six printable ASCII fields with the fixed `WING` marker, a valid IPv4 field, and exact model `ngc-full`. Returned addresses, editable console names, serials, firmware, and raw datagrams remain Agent-local; WING COMPACT/RACK, X32/X AIR/FLOW models, generic OSC/UDP/AES50/Dante behavior, and reachability are safe false negatives. Tenant-scoped pending/completed/failed state and a native manager action are included; no real mixer was contacted and no control or recovery support is credited.
- Soundcraft automatic network identification is deliberately deferred. ViSi Remote automatically discovers consoles over HiQnet, but Soundcraft publishes neither the discovery wire exchange nor exact literal online model replies, and selecting a result opens live mixer controls. HARMAN's generic HiQnet discovery uses UDP broadcast, omits per-device details, and exposes configurable device/network fields; Ui's built-in web server is likewise the full mixer-control application, not a bounded identity endpoint. ShowVault does not probe these paths, submit credentials, weaken access control, or infer identity from generic HiQnet, HTTP, TCP/UDP, OSC, MIDI, Dante, editable names, hotspot SSIDs, discovery, default addresses, or reachability. Existing exact-root Soundcraft Vi showfolder recovery remains separate.
- Tascam automatic network identification is deliberately deferred. Sonicview Control and TASCAM IO CONTROL discover devices through UPnP multicasting but do not publish exact discovery replies, display editable names, and proceed into administrator login and live control/configuration. Tascam's published DA-6400/DA-6400dp and SS-CDR250N/SS-R250N TELNET protocols share the same read-only information request, whose response contains only four software-version digits and no product literal. ShowVault does not broadcast, enter these control sessions, submit credentials, rely on blank/default passwords, or infer identity from generic TELNET, TCP/UDP, UPnP, OSC, MIDI, Dante, names, versions, discovery, default addresses, or reachability. Existing exact-root Model-series `MTR` song recovery remains separate.
...
- Next recommended implementation step: research official primary sources for a bounded, read-only, exact product-identity contract for the Panasonic row under PTZ and cameras. Keep PTZ-camera evidence separate from existing Panasonic projector signatures; preserve the manager-authorized responder boundary and path-free control-plane contract, and do not infer identity from generic AW, VISCA, ONVIF, NDI, HTTP, mDNS, or port reachability.
