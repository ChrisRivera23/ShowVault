...
- `codex/zero-config-candidate-discovery` — venue-agnostic automatic recovery-candidate discovery beginning with standard Resolume locations on macOS and Windows.
...
- Current branch: `codex/zero-config-candidate-discovery`, based on the macOS packaging branch. It adds bounded read-only local recovery candidates to system inventory, detects existing Resolume Arena/Avenue application and user-data locations across standard macOS and Windows user profiles, ignores missing or inaccessible locations, and marks every result as requiring operator approval. Candidate paths remain in the Agent's local discovery result. Control-plane approval records, onboarding UI, durable Agent-local approved scopes, and approved-subnet network discovery remain outstanding.
...
Draft PRs #3 through #11 establish Auth0 tenancy, the Agent-side Scan → Backup → Verify → Restore loop, and tenant recovery history. Later stacked branches add approved vendor recovery integrations through DAS Audio ALMA; RCF, Funktion-One, Adamson, and Outline are explicitly deferred pending documented or fixture-verified complete recovery formats. No pilot venue is selected. ShowVault must install at an unknown venue and discover candidates without pre-entered computer details, paths, addresses, or inventory. System inventory now finds standard Resolume Arena/Avenue application and data locations on macOS and Windows, keeps candidate paths local, and requires approval before protection. Production-style macOS LaunchDaemon packaging remains available. All 255 Agent tests pass. The next milestone is privacy-preserving candidate/approval persistence, native onboarding, durable local approved scopes, and bounded approved-subnet discovery. The catalog remains committed and resumes with KV2 Audio after zero-configuration onboarding is proven. All 7 API tests, Flutter analysis, and all 6 Flutter tests remain at their previously verified baseline.
