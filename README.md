...
- `codex/prototype-workflow-api` — typed operator-facing recovery workflow endpoints.
...
- Exact Codex context-window percentages are not exposed to the assistant. This conversation is estimated at roughly 45–50% after the prototype workflow API slice (unmeasured).
...
Draft PRs #3 through #11 establish Auth0 tenancy, the Agent-side Scan → Backup → Verify → Restore loop, and tenant recovery history. Later stacked branches add approved vendor recovery integrations through Dynacord SONICUE. The prototype workflow API now exposes safe typed operations for the Flutter client. New catalog work is paused only until the prototype is proven; the full Version 1 scope remains committed in `docs/INTEGRATION_CATALOG.md` and will resume with Electro-Voice. All 208 Agent tests and all 7 API tests pass. Flutter workflow controls, live Auth0/API proof, and macOS LaunchDaemon keychain installer validation remain outstanding.
