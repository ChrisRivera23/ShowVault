...
- `codex/sony-broadcast-identification` — official-source research and safety deferral for Sony broadcast-device network identity.
...
- Next recommended implementation step: research official NewTek primary sources for an exact, read-only network identity contract for the broadcast-device catalog. Reuse an existing manager-authorized bounded responder set only if its authorization covers the documented protocol; do not infer identity from generic NDI participation, reachability, open ports, multicast/broadcast advertisements, or mutable control endpoints.
