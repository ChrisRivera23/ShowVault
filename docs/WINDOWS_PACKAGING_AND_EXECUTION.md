...
The correction must therefore use an explicitly immutable dependency source/baseline that contains the complete pinned Auth0 2.6.0 Windows package graph instead of assuming the runner's moving vcpkg ports tree retains `cpprestsdk`. It must preserve the validated local `VCPKG_ROOT`, redirection rejection, closed five-package set, exact triplet, and fail-closed install/result verification. The current macOS host still has no Windows VM/device, PowerShell runtime, Wine environment, Windows SDK/MSVC toolchain, or Inno Setup compiler. The installer has not been compiled or executed, the Auth0 native dependencies have not been resolved by a ShowVault Windows build, the URL callback has not been exercised, and no Windows artifact hash or installed evidence exists. Do not claim Windows packaging or runtime readiness until the controlled command above passes on Windows.
