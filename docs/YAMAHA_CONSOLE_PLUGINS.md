...
The Yamaha console recovery track currently supports four separately validated product families:
...
Configure exact operator-exported directories under `YamahaDm7ExportRoots`, `YamahaRivageExportRoots`, `YamahaClQlExportRoots`, and `YamahaTfExportRoots`. Each root must contain its matching settings artifact. Child-only scans, cross-family formats, and directories without a recognized settings file are rejected.
...
This slice does not yet cover DM3, Rivage system logs, Yamaha DSP processors, network switches, amplifiers, or speakers. Those remain mandatory catalog targets but require their own documented file formats, model/version fixtures, and safe recovery workflows. A passing hash verification proves the export was preserved exactly; it does not prove compatibility with another console model, bus configuration, license state, or firmware version. TF firmware/editor combinations in particular must be checked against Yamaha's compatibility information before loading.
