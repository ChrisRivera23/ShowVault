...
This slice does not cover RIVAGE system logs, network switches, amplifiers, or speakers. Yamaha DME7 and MTX/MRX DSP projects are handled by the separate [Yamaha DSP project plugins](YAMAHA_DSP_PLUGINS.md). The remaining targets require their own documented file formats, model/version fixtures, and safe recovery workflows. A passing hash verification proves the export was preserved exactly; it does not prove compatibility with another console model, bus configuration, license state, or firmware version. TF firmware/editor combinations in particular must be checked against Yamaha's compatibility information before loading.
