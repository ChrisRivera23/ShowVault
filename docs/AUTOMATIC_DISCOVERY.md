...
1. Detect local network interfaces and propose bounded directly connected venue subnets without scanning them; exclude loopback, link-local, tunnel, and obviously non-venue interfaces.
2. Persist explicit subnet approval and permit bounded non-invasive discovery only after approval.
