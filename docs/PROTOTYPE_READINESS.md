...
| Native operator application | The Flutter macOS application performs exact catalog scanning and an installed synthetic local Save/verify/queue through native source and vault pickers. Reopening the synthetic vault after process restart restores verified and queued status without reading the source. A universal release bundle builds successfully with the required user-selected read/write entitlement. | Installed synchronization proof, production authenticated cloud transport, personal-data runtime proof, Windows installed proof, and restore remain. |
...
| Operational resilience | Unit and integration tests cover restart-resume, idempotency, offline backoff, retry exhaustion, local/remote tamper, links, malformed state, and privacy-filtered remote manifests. | A production-like end-to-end failure/restart drill on personal equipment is not yet recorded. |
...
3. Add attended restore from a verified local recovery point into an absent or empty controlled target, then bind the synchronization transport to authenticated hosted storage.
...
Implement attended local restore from a verified recovery point into an absent or empty operator-selected synthetic target. Reverify package identity before copying, reject links and non-empty targets, publish atomically where possible, verify restored bytes independently, persist path-safe restore evidence, and keep restore independent from cloud availability.
