...
| Operational resilience | Unit and integration tests cover restart-resume, idempotency, offline backoff, retry exhaustion, local/remote tamper, links, malformed state, and privacy-filtered remote manifests. An installed synthetic macOS drill proves process interruption/resume and offline retry through the authenticated API. | Host reboot, production-like deployment/object storage, broader failure/tamper automation, and personal-data recovery remain unproven. |
...
Replace the development-only startup and server-filesystem storage assumptions with a versioned, deployable prototype environment and a production-object-storage adapter behind the existing hosted-sync contract. Define configuration, credential boundaries, tenant key layout, resumable-write/commit semantics, retention and migration procedures, local-development parity, and failure observability. Preserve the installed app protocol, local-first guarantees, path-free manifests, authorization checks, and a controlled filesystem backend for tests; do not claim regional durability until deployment evidence exists.
