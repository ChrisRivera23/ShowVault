...
| Platform coverage | Direct exact-location scan definitions include macOS and Windows candidates for the current beta products. A current-user x64 installer definition, clean package/checksum script, controlled installed-proof runner, explicit local-drive/junction/diagnostic path policies, and manual read-only-permission Windows-native evidence workflow are versioned and covered by host-independent tests. | No Windows build/runtime was available and the workflow was not pushed or dispatched. Native PowerShell/Inno validation, package creation, callback registration, NTFS junction behavior, installed recovery execution, attended picker/Auth0 behavior, and artifact evidence remain unproven. |
