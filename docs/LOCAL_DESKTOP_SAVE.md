...
- A resumable, idempotent, checksum-verifying executor now uses an authenticated, tenant-authorized ShowVault API transport in normal signed-in builds. The first server backend is an explicitly configured controlled filesystem root; production object-storage retention, regional durability, bandwidth policy, and distributed locking remain unimplemented.
